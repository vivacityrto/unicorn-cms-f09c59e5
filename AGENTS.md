...
- Tests (P0.2, `docs/kb/reference/codebase-optimization-plan-2026-08-28.md`):
  ...
  as of Phase 2.6 stabilization Packet P1-B (2026-09-07) it reports **zero**
  errors — the single earlier `ContactDirectory.tsx` TS2345 error noted here
  at P0.2 was fixed independently before this packet started, and this
  packet fixed the two files that had since become the live baseline
  (`ClientLayout.tsx`'s realtime payload narrowing, `useKpiSummary.tsx`'s
  dynamic-relation-name dispatch). `.github/workflows/typecheck.yml` now
  runs `npm run typecheck` on every PR and fails the check on any error —
  there is no stale-baseline exception. `npm run typecheck` **is not** `tsc -b --noEmit`: that
