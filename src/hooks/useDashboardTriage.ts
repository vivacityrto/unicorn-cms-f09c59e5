...
export function useDashboardTriage() {
  ...
  const canSeeAll = isVivacityStaffRole(profile?.unicorn_role);
  ...
  const { data: rawTenants = [], isLoading: tenantsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!isSuperAdmin && profile?.user_uuid) {
        // Non-SA My Clients: filter to assigned tenants only
        if (savedView === 'my_tenants') {
          query = query.eq('assigned_csc_user_id', profile.user_uuid);
        }
        // Non-SA All Clients: no filter — view returns all active tenants
      }
  ...
  const { data: backendPrompts = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!isSuperAdmin && savedView === 'my_tenants' && profile?.user_uuid) {
        query = query.eq('owner_user_id', profile.user_uuid);
      }
      const { data, error } = await query;
      if (error) throw error;
      return (data || []) as unknown as PriorityInboxItem[];
