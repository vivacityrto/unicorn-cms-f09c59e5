  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('clients_legacy').select('id, companyname, rto_name, rtoid, logo_url').eq('tenant_id', profile!.tenant_id!).order('companyname');
      if (error) throw error;
      return data;
    },
    enabled: !!profile?.tenant_id
  ...
  const inspections: AuditInspection[] = useMemo(() => {
    ...
    return audits.map((audit: any) => {
      ...
      return {
        ...
        score: audit.status === 'complete' ? Math.random() * 50 + 50 : audit.status === 'in_progress' ? Math.random() * 40 : undefined,
  ...
  return <DashboardLayout>
      <div className="space-y-6 p-6 animate-fade-in w-full">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-[28px] font-bold">Compliance Audits</h1>
            <p className="text-muted-foreground">Manage compliance, inspections, schedules, and analytics in one place.</p>
          </div>
          {activeTab === 'templates' && <Button onClick={handleCreateTemplate} className="bg-[hsl(188_74%_51%)] hover:bg-[hsl(188_74%_51%)]/90 gap-2">
              <Plus className="h-4 w-4" />
              Create Template
            </Button>}
          {activeTab === 'inspections' && <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
              ...
              <DialogContent>
                ...
                <div className="space-y-4">
                  ...
                  <Button onClick={handleCreateAudit} disabled={!selectedClient || createAudit.isPending} className="w-full">
                    Create Audit
                  </Button>
                </div>
              </DialogContent>
        ...
        {/* Tab Content */}
        {activeTab === 'templates' && <AuditTemplatesTable templates={templates || []} isLoading={templatesLoading} onCreateTemplate={handleCreateTemplate} onBrowseLibrary={handleBrowseLibrary} onStartInspection={handleStartInspection} onEditTemplate={t => navigate(`/audits/create-template/${t.id}`)} onDuplicateTemplate={handleDuplicateTemplate} onDeleteTemplate={handleDeleteTemplate} />}

        {activeTab === 'inspections' && <AuditInspectionsTable inspections={inspections} isLoading={isLoading} />}

        {activeTab === 'schedules' && <Card>
            <CardContent className="p-12 text-center">
              <ClipboardCheck className="h-12 w-12 mx-auto mb-4 text-muted-foreground" />
              <h3 className="text-lg font-semibold mb-2">No schedules yet</h3>
              <p className="text-muted-foreground">
                Schedule your upcoming audits here
              </p>
            </CardContent>
          </Card>}
        ...
        {activeTab === 'analytics' && <Card>
            <CardContent className="p-12 text-center">
              ...
              <p className="text-muted-foreground">
                View audit insights and reports
              </p>
            </CardContent>
          </Card>}
      </div>
