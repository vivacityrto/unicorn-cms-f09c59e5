  ...
  const { data: lesson, isLoading: lessonLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("id, module_id, course_id, title, description, lesson_type, sort_order, is_published, is_preview, estimated_minutes, video_id, resource_id, content_markdown, completion_threshold, segment_start_seconds, segment_end_seconds")
  ...
  const { data: modules = [] } = useQuery({
    ...
    queryFn: async () => {
      const [{ data: mods, error: mErr }, { data: lessons, error: lErr }] = await Promise.all([
        ...
        supabase.rpc("get_academy_course_lesson_outline_safe", { p_course_ids: [course!.id] }).select("id, module_id, title, lesson_type, sort_order, is_published, is_preview, estimated_minutes").order("sort_order"),
      ]);
      if (mErr) throw mErr;
      if (lErr) throw lErr;
