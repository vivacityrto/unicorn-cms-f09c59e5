...
const ResourceMostUsed = () => {
  ...
  const { data: mostUsedResources = [], isLoading } = useMostUsedResources(50);
  ...
  const availableTags = useMemo(() => {
    ...
    mostUsedResources.forEach((r) => r.tags.forEach((t) => tagSet.add(t)));
    return Array.from(tagSet).sort();
  }, [mostUsedResources]);

  const isSearching = searchTerm.length >= 2 || selectedCategory || selectedTags.length > 0;
  const displayResources = isSearching ? searchResults : mostUsedResources;
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Most Popular</h1>
            <p className="text-muted-foreground">
              Browse the most viewed and downloaded resources
            </p>
          </div>
        </div>
...
export default ResourceMostUsed;
