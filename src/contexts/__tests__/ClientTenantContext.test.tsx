...
describe("ClientTenantContext gating", () => {
  it("primary contact, full scope -> portal + manage users true", async () => {
    dbState.tenantUserRows = [
      { tenant_id: 7532, access_scope: "full", primary_contact: true, secondary_contact: false },
    ];
    renderCtx();
  ...
  it("secondary contact, full scope -> portal + manage users true", async () => {
    dbState.tenantUserRows = [
      { tenant_id: 7532, access_scope: "full", primary_contact: false, secondary_contact: true },
    ];
    renderCtx();
    await waitFor(() => expect(captured?.tenantUserLoading).toBe(false));
    expect(captured?.canAccessClientPortal).toBe(true);
  ...
  it("academy_only scope -> isAcademyOnly true, portal false", async () => {
    dbState.tenantUserRows = [
      {
        tenant_id: 7532,
        access_scope: "academy_only",
  ...
  it("resilient resolution: users.tenant_id null + exactly one tenant_users row -> resolves & grants secondary access", async () => {
    ...
    dbState.tenantUserRows = [
      { tenant_id: 9001, access_scope: "full", primary_contact: false, secondary_contact: true },
    ];
    renderCtx();
  ...
  it("multi-row defensiveness: users.tenant_id null + 2 tenant_users rows -> activeTenantId stays null and warns", async () => {
    ...
    dbState.tenantUserRows = [
      { tenant_id: 9001, access_scope: "full", primary_contact: false, secondary_contact: true },
      { tenant_id: 9002, access_scope: "full", primary_contact: true, secondary_contact: false },
    ];
    renderCtx();
    await waitFor(() => {
