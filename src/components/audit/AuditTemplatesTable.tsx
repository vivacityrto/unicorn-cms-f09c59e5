...
export function AuditTemplatesTable({
  ...
  const statusOptions = [
    ...
    { value: 'locked', label: 'Locked', icon: Lock, color: 'text-amber-600' },
    { value: 'draft', label: 'Draft', icon: FileEdit, color: 'text-blue-600' },
  ];
  ...
  const getStatusBadge = (status: AuditTemplate['status']) => {
    switch (status) {
      case 'active':
        return (
          <Badge className="bg-green-500/10 text-green-600 hover:bg-green-500/20 border border-green-600 text-[0.75rem] py-[2px] px-[0.625rem] rounded-[11px] gap-1">
      ...
      case 'locked':
        return (
          <Badge className="bg-amber-500/10 text-amber-600 hover:bg-amber-500/20 border border-amber-600 text-[0.75rem] py-[2px] px-[0.625rem] rounded-[11px] gap-1">
      ...
      case 'draft':
        return (
          <Badge className="bg-blue-500/10 text-blue-600 hover:bg-blue-500/20 border border-blue-600 text-[0.75rem] py-[2px] px-[0.625rem] rounded-[11px] gap-1">
                    ...
                    <TableCell className="py-6 border-r border-border/50 min-w-[300px] pr-8">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0">
                          {template.icon_url ? (
                            <img src={template.icon_url} alt="" className="h-6 w-6 object-contain" />
                          ) : (
                        ...
                        <div className="min-w-0">
                          ...
                          {template.created_at && (
                            <p className="text-[13px] text-muted-foreground truncate mt-1 flex items-center gap-1.5">
                              <Calendar className="h-3.5 w-3.5" />
                              Created: {new Date(template.created_at).toLocaleDateString('en-AU', { day: '2-digit', month: '2-digit', year: 'numeric' }).replace(/\//g, '/')}
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    <TableCell className="py-6 px-4 text-center whitespace-nowrap">
                      <div className="flex items-center justify-center gap-2">
                        <Button 
                          ...
                          className="whitespace-nowrap hover:bg-[hsl(196deg_100%_93.53%)] hover:text-black hover:border-[#00000052]"
                        >
