    ...
    <div className="flex flex-wrap gap-2 mt-2">
      {attachments.map((a) => {
        if (isImageMime(a.mime_type)) {
          ...
          if (!url) {
            return <Skeleton key={a.id} className="h-24 w-32 rounded-md" />;
          }
          return (
            <img
              key={a.id}
              ...
              className="max-w-[200px] max-h-[200px] rounded-md cursor-pointer border border-border object-cover"
            />
          );
        }
        ...
        if (isPdfMime(a.mime_type)) {
          return (
            <button
              ...
              className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-border bg-background hover:bg-muted text-xs"
            >
