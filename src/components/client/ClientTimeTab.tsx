...
function ClosedPackageSummaries({ tenantId }: { tenantId: number }) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return closedInstances.map((inst: any) => ({
        ...
        ...(totalsMap[inst.id] || { billable: 0, nonBillable: 0, total: 0 }),
      }));
    },
    staleTime: 60_000,
  });
  ...
  return (
    <div className="mt-4">
      ...
      >
        {expanded ? 'Hide' : 'Show'} closed packages ({data.length})
      </button>
      {expanded && (
        <div className="space-y-1.5">
          {data.map(pkg => {
            const billPct = pkg.total > 0 ? Math.round((pkg.billable / pkg.total) * 100) : 0;
            return (
              <div key={pkg.id} className="flex items-center justify-between gap-3 px-3 py-2 bg-muted/50 rounded-md text-[11px]">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="font-medium truncate">{pkg.name}</span>
                  <span className="text-muted-foreground whitespace-nowrap">
                    {formatLifecycle(pkg.start_date, pkg.end_date)}
                  </span>
                </div>
                <div className="flex items-center gap-4 shrink-0">
                  <span className="text-green-600">
                    Bill: {formatDuration(pkg.billable)} ({billPct}%)
                  </span>
                  <span className="text-muted-foreground">
                    Non-bill: {formatDuration(pkg.nonBillable)}
                  </span>
