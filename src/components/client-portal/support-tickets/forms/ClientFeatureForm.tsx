...
export type ClientFeatureFormValues = z.infer<typeof schema>;
export interface FormHandle { submit: () => Promise<ClientFeatureFormValues | null>; }

export const ClientFeatureForm = forwardRef<FormHandle>((_, ref) => {
  const form = useForm<ClientFeatureFormValues>({
  ...
  return (
    <form className="space-y-4">
      ...
      <div className="space-y-1.5">
        <Label htmlFor="feature_context">Any additional context?</Label>
        <Textarea id="feature_context" rows={3} {...register('feature_context')} />
      </div>
    </form>
  );
});
ClientFeatureForm.displayName = 'ClientFeatureForm';
