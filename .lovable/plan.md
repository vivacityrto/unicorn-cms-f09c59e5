## Goal
Rewrite `public.audit_invalid_memberships()` so each broken membership row appears once with all its issues concatenated, instead of duplicating rows across the four checks.

## Migration

Replace the function body with a single CTE wrapping the existing four `UNION ALL` branches, then group by membership identity:
...
## Out of scope
No changes to other functions, tables, hooks, or frontend pages.
