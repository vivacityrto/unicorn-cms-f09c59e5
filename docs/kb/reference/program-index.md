...
> **Last updated:** 2026-09-16 · **Reconsider by:** 2026-10-10 · **Confidence:** high (status lines below are read from each initiative's own master doc header and the latest merged phase/packet evidence, not inferred).
...
| RBAC v6 | planning | All 15 §13 items dispositioned (item 1 permanently decided 2026-09-11 via ADR-030; items 2-13 baselined 2026-09-10; items 14/15 parked). Packet P0.1 and the broad P1 preparation set through P1-u are delivered. R2-a vertical-slice work has produced **four confirmed-golden rows** (`eos.scorecard.manage` [P1-w], `eos.rocks.own.manage` [P1-x], `eos.configurations.manage` [P1-y], `admin.permissions.manage` [P1-z], all confirmed 2026-09-15) plus a chain of same-day security fixes to core authorization primitives found while tracing them — see the Active-work row below and `docs/audit-log/INDEX.md`'s 2026-09-15 entries for the full list. **A fifth candidate, `admin.vector.manage`, is scoped but its golden-matrix packet is not yet drafted — that's the next concrete RBAC v6 task for a fresh session.** Two §13 item-2 sub-decisions executed same-day (PR #1347): CET retired; Team Leader retired in the catalogue but kept in frontend role arrays (load-bearing per a real test regression). Multi-role infrastructure (`user_roles`/`check_permission()`) confirmed already live, not a new build. Capability rows, role defaults, pilot, shadow telemetry, grants, hosted runs, and enforcement remain preparation-only, not approved. The [remaining gated approval packet](codebase-optimization/cross-cutting/remaining-gated-approval-packets-2026-09-12.md) names the actual product/security inputs blocking that. | [`rbac-v6-authorization-implementation-plan-2026-09-01.md`](rbac-v6-authorization-implementation-plan-2026-09-01.md) |
| Tenant Operating Model | planning | All 13 §18 decisions are closed. P0.1/P0.2/P0.3 evidence and owner dispositions are complete. P1.1 contact-promotion canary evidence passed. P1.2-b's guarded ghost-contact snapshot and P1.2-d's approved one-row plus 5/5/4 `unicorn-qa` apply batches passed with redacted evidence, per-batch postflight, zero out-of-scope writes, and a terminal classifier showing zero eligible candidates. P1.2-c's repository retirement and hosted deletion are complete and immediately verified; the 60-minute observation was waived after the prior in-window redeployment invalidated the earlier observation. The next bounded TOM queue is the read-only/documentation owner-review pass for P0.2/P0.3 evidence and the Realtime publication-gap disposition; contact insertion, migration, unmatched-row conversion, Realtime repair, and broader runtime work remain separately gated. Resume from the [Codex pause/resume handoff](tenant-operating-model/p1/tom-codex-pause-resume-handoff-2026-09-16.md). | [`tenant-operating-model-data-architecture-plan-2026-09-02.md`](tenant-operating-model-data-architecture-plan-2026-09-02.md) |
...
| RBAC v6 | **Four vertical slices confirmed golden 2026-09-15 (Carl):** [P1-w](rbac-v6/p1/p1-w-eos-scorecard-golden-matrix-draft.md) `eos.scorecard.manage`, [P1-x](rbac-v6/p1/p1-x-eos-rocks-own-manage-golden-matrix-draft.md) `eos.rocks.own.manage`, [P1-y](rbac-v6/p1/p1-y-eos-configurations-manage-golden-matrix-draft.md) `eos.configurations.manage`, [P1-z](rbac-v6/p1/p1-z-admin-permissions-manage-golden-matrix-draft.md) `admin.permissions.manage`. Tracing them surfaced a chain of real, same-day security fixes to core authorization primitives, all merged and (as of a full CI redeploy dispatched this session) live: a `SECURITY DEFINER` RLS-bypass in `upsert_rock_with_parenting` (PR #1354); a disabled/archived-account bypass in `is_super_admin_safe`/`has_permission`/`check_permission` (PR #1363); the same class of gap in `is_tenant_parent_safe`/`has_tenant_admin_safe`, found via a Codex-requested RBAC review of TOM's replacement invitation path (PR #1366); and a 4-instance sweep of a dead `profile.state` account-status comparison (`public.users.state` is `bigint`, not account status) across `verifyAuth`, `ask-viv-access.ts`, `requireCaller.ts`'s `requireSuperAdmin`, and `backfill-vimeo-durations` (PRs #1370, #1380). See `docs/audit-log/INDEX.md`'s 2026-09-15 entries for full detail on each. Also built this session: a CI-based Edge Function auto-deploy system (`.github/workflows/deploy-edge-functions.yml` + `scripts/select-affected-edge-functions.mjs`, PRs #1382/#1383) replacing the unreliable native Supabase dashboard sync and the expensive manual redeploy process the security fixes above required — it follows shared-file import dependencies transitively, so a future fix to a widely-used file redeploys every real importer automatically; manual dispatch (`gh workflow run deploy-edge-functions.yml [-f functions=...]`) is also available for ad-hoc catch-up deploys. **Next step for a fresh session:** draft the golden-matrix packet for the fifth candidate, `admin.vector.manage` — Carl already decided it's hard-Super-Admin-locked, no exceptions, per P1-e; verify that fresh against live RLS/`role_permissions`/the Edge Function auth gate, following the P1-w/x/y/z format, then present for Carl's confirmation. This was scoped but not drafted — investigation was interrupted mid-way by the security findings above. | — | Claude Code | 2026-09-15 |
| Tenant Operating Model | P1.2-c complete; work paused for Carl's temporary ComplyHub focus. Resume from the [Codex pause/resume handoff](tenant-operating-model/p1/tom-codex-pause-resume-handoff-2026-09-16.md) | — | Codex | 2026-09-16 |
...
- [2026-09-16 — TOM P1.2-c observation aborted by in-window redeployment](../../audit-log/entries/2026-09-16-tom-p12c-observation-aborted-redeploy.md) — the interim final read-only check found inventory back at 193 and `activate-ghost-user` present/ACTIVE again after an in-window redeployment; exact-window request logs and audit events remained zero. That interim exception was reconciled by the repository retirement and final closeout below.
- [2026-09-16 — TOM P1.2-c retirement closeout](../../audit-log/entries/2026-09-16-tom-p12c-retirement-closeout.md) — after PR #1381 merged, the hosted function was permanently deleted and immediately verified absent (192 functions); replacement guards remained active, activation jobs were safe, and recent target requests/audit events were zero; Carl's 60-minute observation waiver is recorded.
- [2026-09-16 — TOM P1.2-c repository retirement](../../audit-log/entries/2026-09-16-tom-p12c-repository-retirement.md) — removed the legacy function source/configuration and current repository references in `b771e3f7e`; hosted deletion and immediate verification remain separate control-plane steps, with the 60-minute observation waived.
- [2026-09-16 — TOM P1.2-c production `activate-ghost-user` deletion and observation](../../audit-log/entries/2026-09-16-tom-p12c-production-deletion-and-observation.md) — explicit action-time approval authorized deletion from production after read-only preflight; the initial inventory fell 193→192 and the exact lookup was absent before the later in-window redeployment invalidated the observation.
...
- [2026-09-15 — TOM P1.2-c aggregate legacy-profile classification and log-retention gap](../../audit-log/entries/2026-09-15-tom-p12c-aggregate-legacy-profile-classification.md) — all-source sweep found no retained rows on any day from June 3–16 (a retention gap, not zero-use proof); 411 unmatched profiles classified in aggregate as 356 membership-bearing and 55 membershipless; no identifiers or live state changes.
- [2026-09-15 — TOM P1.2-c operational retirement packet prepared](../../audit-log/entries/2026-09-15-tom-p12c-retirement-operational-packet.md) — exact target metadata captured read-only for ACTIVE v304; disable-first rollback packet prepared with Carl-owned execution gates; no hosted action occurred and retirement remains unauthorized.
- [2026-09-15 — TOM P1.2-c retirement packet target approved as `unicorn-qa` only](../../audit-log/entries/2026-09-15-tom-p12c-retirement-qa-target-approval.md) — Carl approved the allowlisted QA target and explicitly excluded production; target selection only, with no hosted action authorized.
- [2026-09-15 — TOM P1.2-c disable-first action shape approved](../../audit-log/entries/2026-09-15-tom-p12c-retirement-action-shape-approval.md) — Carl approved the reversible disable-first sequence in QA; deletion remains deferred and execution remains unauthorized.
- [2026-09-15 — TOM P1.2-c supervised QA timing approved](../../audit-log/entries/2026-09-15-tom-p12c-retirement-timing-approval.md) — Carl approved the next supervised QA window immediately after a passing preflight; no unattended run or hosted action is authorized.
- [2026-09-15 — TOM P1.2-c rollback owner reconciled](../../audit-log/entries/2026-09-15-tom-p12c-retirement-rollback-owner.md) — ADR-028's existing Carl-owned rollback decision applies to this run; no delegation or hosted action is authorized.
- [2026-09-15 — TOM P1.2-c observation window approved](../../audit-log/entries/2026-09-15-tom-p12c-retirement-observation-window.md) — Carl approved a minimum 60-minute post-disable observation; early rollback remains allowed on thresholds and deletion remains separately gated.
- [2026-09-15 — TOM P1.2-c final preflight aborted on QA target mismatch](../../audit-log/entries/2026-09-15-tom-p12c-retirement-preflight-abort.md) — approved `unicorn-qa` currently lacks the historical target function; no project switch or hosted action occurred.
...
- [2026-09-15 — `has_permission`/`check_permission`/`is_super_admin_safe` never excluded disabled accounts](../../audit-log/entries/2026-09-15-gate-permission-core-on-disabled-and-archived.md) — the three core RBAC v6 permission-check functions never (or only partly) tested `disabled`/`archived`; confirmed live and exploitable (a disabled CSC account retained `full`-level grants); fixed same-day, zero regression for active accounts.
- [2026-09-15 — `is_tenant_parent_safe`/`has_tenant_admin_safe` never excluded disabled accounts](../../audit-log/entries/2026-09-15-gate-tenant-parent-and-admin-safe-on-disabled.md) — the same class of gap in the client-tenant-side authorization functions behind the contact swap/promote RPCs and `user_invitations`' own RLS; 359 and 338 currently-affected accounts respectively; fixed same-day.
- [2026-09-15 — `verifyAuth`'s account-status check was silently dead across 16 Edge Functions](../../audit-log/entries/2026-09-15-fix-verify-auth-dead-account-status-check.md) — compared a `bigint` column to string literals, so it could never fire; 10 of 16 consumer functions have no other backstop; 10 real disabled/archived internal-staff accounts exist today. Source fixed, tested, and deployed.
- [2026-09-15 — Two more dead `profile.state` account-status checks, plus root-cause type fix](../../audit-log/entries/2026-09-15-fix-remaining-state-check-instances.md) — an exhaustive grep found the same bug in `requireCaller.ts`'s widely-shared `requireSuperAdmin()` gate and `backfill-vimeo-durations`; root cause was `public.users.state` being `bigint` (Australian state/territory data), hand-mistyped as a string account-status field and copy-pasted 4 times total with zero CI type-checking to catch it. Both fixed; type declaration corrected so the mistake can't look valid again.
- [2026-09-15 — Reliable, dependency-aware Edge Function auto-deploy via CI](../../audit-log/entries/2026-09-15-edge-function-ci-auto-deploy.md) — replaces Supabase's unreliable native dashboard GitHub-sync with a repo-owned workflow that follows every function's local imports transitively to compute the real affected set on each push, then deploys via the Supabase CLI (bundles directly from the checked-out filesystem — no hand-typed file list anywhere). Built after the security fixes above required an expensive, error-prone manual redeploy sweep.
