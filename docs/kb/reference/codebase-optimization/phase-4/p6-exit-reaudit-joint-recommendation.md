...
| `AdminStageDetail.tsx` | 2,703 (2,574 per Codex's independent read — both above the plan's threshold either way) | **No existing ownership claim found by either agent.** | Needs a small joint ownership matrix (RBAC: staff/client capability boundaries; TOM: tenant/package/stage identity and source-of-truth semantics; Client Health: only if a proven consumer exists) before any state/query/mutation extraction. Pure UI seams can stay Codebase-owned if genuinely policy-neutral — that's the one candidate here where a narrow Codebase slice might still be viable, scoped only to what the matrix clears. |
