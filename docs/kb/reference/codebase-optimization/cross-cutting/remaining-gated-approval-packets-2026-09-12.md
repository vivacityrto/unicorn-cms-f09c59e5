...
| R2-f: AJ/CSC pilot | Pilot only the smallest demonstrated Academy/package/stage action set, with named scope, personas, 14-day shadow, zero-tolerance mismatch gates, and rollback owner | Pilot scope is a product decision and the current route allowlist is not a server policy | No pilot enrollment, grant, route cutover, or production observation |
