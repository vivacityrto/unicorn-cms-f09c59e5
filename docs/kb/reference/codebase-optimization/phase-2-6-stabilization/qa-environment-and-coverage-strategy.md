...
| `qa:e2e` | Authenticated read-only route and workflow smoke checks | Route, auth or query-behavior change | First target written and live-proven, 4/4 passing (local run, not CI-gated -- see below). Persistent (not per-run) `qa-e2e-demo-tenant` + Super Admin/client personas seeded by `scripts/qa-seed-e2e-personas.mjs`. See `progress-log.md` session 38 for the full discovery: `src/integrations/supabase/client.ts` hardcodes production's URL/anon key (not env-driven), so a run requires temporarily editing those two literals in an isolated worktree, generating storage states, running `npm run e2e:qa`, then reverting before anything is committed -- never do this on a shared checkout or commit a QA-pointed `client.ts`. Also requires seeding `tenant_users` (not just `tenant_members` -- `ClientTenantContext.tsx` gates portal access on the former) and populating two empty QA reference tables (`dd_relationship_role`, `dd_unicorn_roles`'s `Admin` row) |
