...
| `qa:edge` | Auth, CORS, request/response and external-contract behavior | Edge Function change | First target (auth/CORS/method/response-contract tests against `tenant-lifecycle`) live-proven, 6/6 passing (workflow run `34290824352`) — see `progress-log.md` session 35. Broadening to other functions means deploying each to `unicorn-qa` first — the same decision already made once for `tenant-lifecycle` |
| `qa:data-lifecycle` | Create/update/archive/delete workflows and invariants | Feature workflow or trigger change | First target (tenant lifecycle: suspend/close/archive/reactivate) live-proven, 11/11 passing (workflow run `34289646607`) — see `progress-log.md` sessions 32-33. Along the way, live-verifying it found and fixed a real production bug (L10 item 34: the Close action failed for every tenant, unconditionally, since the code was written) |
| `qa:residue` | Run-scoped rows, Auth users, storage objects and orphan records | Any fixture-producing suite | Live-proven, 5/5 passing (workflow run `34292052765`) — an independent post-hoc sweep (separate code path from each suite's own cleanup) for `public.users`/`auth.users` rows with a fixture (`@example.test`) email, and `tenants`/`tenant_conversations`/`tenant_messages` rows matching the shared `vitest` RUN_ID marker every fixture-producing suite uses. Known gap: true orphan detection (a child row surviving its parent's delete) for `tenant_members`/`conversation_participants`/`audit_events`/`client_audit_log` isn't checked directly, only transitively via their parent rows. See `progress-log.md` session 37 |
| `qa:migrations` | Replay safety and absence of production URLs, cron, HTTP or hidden backfills | Migration change | **Static-safety half already covered** by `scripts/audit-migrations.mjs`, live in CI on every PR/push touching migrations (`.github/workflows/migration-safety.yml`, diff-scoped). **Dynamic-replay half demonstrated** (2026-09-09, `progress-log.md` session 39): `supabase/migrations/20260908080000_retire_forecast_health_crons.sql` (a real, already-merged migration -- Codex's M4 forecast-cron retirement) was replayed onto `unicorn-qa` via `apply_migration` and confirmed clean: its own guard (`pg_cron` absent → early-return, no error) executed correctly, and `qa_cron_safety_status()` still reported zero jobs afterward. This is **not** wrapped in a new bespoke Vitest suite: the ledger is a controlled baseline-replay ledger, not a 1:1 mirror of production's migration history (confirmed -- every existing entry is a `qa_baseline_*`/`qa_seed_*`-style name, never a real migration filename), so a name-matching "drift" test would assert the wrong invariant. This migration's actual guarantee (unicorn-qa stays schedule-free) is already covered as an ongoing regression by the existing `qa:cron-safety` suite -- a future replay that left a job scheduled would fail that suite, not a new one. The "explicit QA sync" step itself (§ below) remains a manual/reviewed action, by design, not something CI performs unattended |
| `qa:e2e` | Authenticated read-only route and workflow smoke checks | Route, auth or query-behavior change | First target written and live-proven, 4/4 passing (local run, not CI-gated -- see below). Persistent (not per-run) `qa-e2e-demo-tenant` + Super Admin/client personas seeded by `scripts/qa-seed-e2e-personas.mjs`. `progress-log.md` session 38 discovered that `src/integrations/supabase/client.ts` hardcoded production's URL/anon key (not env-driven), requiring a hand-edit-and-revert dance for every run. **Closed** (2026-09-11, `hotfix/env-driven-supabase-qa-client`): `client.ts` now reads `import.meta.env.VITE_SUPABASE_URL`/`VITE_SUPABASE_PUBLISHABLE_KEY` at runtime, falling back to the hardcoded production values when unset -- see `playwright.qa.config.ts`'s header comment for the env-var invocation. A run no longer requires editing or reverting `client.ts`; it still requires seeding `tenant_users` (not just `tenant_members` -- `ClientTenantContext.tsx` gates portal access on the former) and populating two empty QA reference tables (`dd_relationship_role`, `dd_unicorn_roles`'s `Admin` row). A future Lovable remix may still regenerate `client.ts` back to hardcoded-only literals -- reapply the env-var read if so. |
| `qa:cron-safety` | QA remains schedule-free unless explicitly enabled | Cron or scheduling change | Live-proven, 2/2 passing (workflow run `34291546797`) — a minimal QA-only `public.qa_cron_safety_status()` RPC (SECURITY DEFINER, EXECUTE granted to `service_role` only, revoked from `anon`/`authenticated`) was added directly to `unicorn-qa` to make "zero pg_cron jobs" a live assertion rather than a one-time baseline observation. QA-only tooling, not part of the production schema — deliberately not in `supabase/migrations/**` or the generated types. See `progress-log.md` session 36 |
