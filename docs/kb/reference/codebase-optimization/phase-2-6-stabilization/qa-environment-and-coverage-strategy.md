...
| `qa:cron-safety` | QA remains schedule-free unless explicitly enabled | Cron or scheduling change | First target written — a minimal QA-only `public.qa_cron_safety_status()` RPC (SECURITY DEFINER, EXECUTE granted to `service_role` only, revoked from `anon`/`authenticated`) was added directly to `unicorn-qa` to make "zero pg_cron jobs" a live assertion rather than a one-time baseline observation. QA-only tooling, not part of the production schema — deliberately not in `supabase/migrations/**` or the generated types. See `progress-log.md` for live-proof status |
