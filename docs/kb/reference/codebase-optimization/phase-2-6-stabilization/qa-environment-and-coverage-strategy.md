...
| `qa:edge` | Auth, CORS, request/response and external-contract behavior | Edge Function change | First target written — auth/CORS/method/response-contract tests against `tenant-lifecycle` (the one function deployed to `unicorn-qa`, for `qa:data-lifecycle`). See `progress-log.md` for live-proof status. Broadening to other functions means deploying each to `unicorn-qa` first — the same decision already made once for `tenant-lifecycle` |
