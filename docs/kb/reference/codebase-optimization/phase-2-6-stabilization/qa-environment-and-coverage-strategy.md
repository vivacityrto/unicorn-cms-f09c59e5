...
| `qa:residue` | Run-scoped rows, Auth users, storage objects and orphan records | Any fixture-producing suite | First target written — an independent post-hoc sweep (separate code path from each suite's own cleanup) for `public.users`/`auth.users` rows with a fixture (`@example.test`) email, and `tenants`/`tenant_conversations`/`tenant_messages` rows matching the shared `vitest` RUN_ID marker every fixture-producing suite uses. Known gap: true orphan detection (a child row surviving its parent's delete) for `tenant_members`/`conversation_participants`/`audit_events`/`client_audit_log` isn't checked directly, only transitively via their parent rows. See `progress-log.md` for live-proof status |
