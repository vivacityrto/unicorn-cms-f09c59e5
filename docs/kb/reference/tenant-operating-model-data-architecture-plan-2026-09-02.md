...
> **Status:** council-reviewed implementation plan; P0.1 evidence and owner-disposition work is complete; P0.2/P0.3 expanded bounded read-only characterization and versioned production metadata cutoff completed 2026-09-13; P1.1 contact-promotion canary and the first P1.2-b guarded ghost-contact QA snapshot completed 2026-09-15; the P1.2-d apply-design packet is prepared; repository source/configuration retirement for `activate-ghost-user` is committed in `b771e3f7e` (PR #1381), with hosted deletion and immediate read-only verification pending after merge; Carl waived the 60-minute observation; contact insertion, migration, and broader runtime implementation gates remain separate and open
> **Prepared:** 2026-09-02
...
> **Audit entry:** non-code production retirement is recorded in [2026-09-16 TOM P1.2-c production deletion and observation](../../audit-log/entries/2026-09-16-tom-p12c-production-deletion-and-observation.md), with repository retirement recorded in [2026-09-16 TOM P1.2-c repository retirement](../../audit-log/entries/2026-09-16-tom-p12c-repository-retirement.md); implementation PRs record their own entries per the Production rule above
...
The follow-up [P1.2-d guarded apply-design packet](tenant-operating-model/p1/p1-2-d-ghost-contact-apply-design-packet.md)
now defines a one-row QA contact-projection canary, frozen-input and
identifier-bearing-manifest requirements, concurrency/idempotency rules,
rollback boundaries, and approval gates. It contains no write-capable
implementation and does not authorize the canary.
...
verification remain. Carl waived the 60-minute observation, and no contact
...
1. ~~Does every active internal Vivacity staff role retain all-tenant read access, or will RBAC v6 introduce portfolio/capability scope?~~ **Decided 2026-09-11 (ADR-030):** every active internal staff member keeps all-tenant read access permanently; RBAC v6 will not introduce portfolio/assignment-scoped read narrowing. Sensitive writes/actions remain separately capability- and scope-controlled. Same shared decision as RBAC v6 plan §13 item 1 -- recorded once. See [decision-trail.md#adr-030](decision-trail.md#adr-030). Item 13's own text below already said "closes all 13 TOM §18 decisions" on 2026-09-10, a day before this item was actually resolved -- this item was the true last of the 13 to close, not item 13.
...
3. ~~Is `tenants.id` the long-term canonical internal key, with `id_uuid` an integration-safe identifier, or is a future key migration required for a specific reason?~~ **Decided 2026-09-10 (ADR-018):** `tenants.id` ratified as the canonical key; `id_uuid` is mandatory (not optional/case-by-case) for every future external integration contract; no UUID-primary migration is planned or justified. See [decision-trail.md#adr-018](decision-trail.md#adr-018).
4. ~~What do the 758 `tenant_profile` IDs, 349 unmatched `tenant_members`, and 25 unmatched `package_instances` represent?~~ **Decided 2026-09-10 (ADR-020):** all three are migration-restore artifacts from unremapped tenant-ID renumbering events, approved for quarantine (not deletion). See [decision-trail.md#adr-020](decision-trail.md#adr-020) for the per-table evidence, including the one ambiguous `package_instances` row flagged for manual review.
5. ~~Which of `tenant_users` and `tenant_members` is authoritative for membership, contacts, invitations, and client administration?~~ **Decided 2026-09-10 (ADR-019):** `tenant_members` ratified as canonical (it already models invited/active status; 175 RLS policies vs. 32 depend on it); `tenant_users`' contact-relationship columns (primary/secondary contact, access scope, relationship role, position type) migrate onto it rather than remaining a second access authority. See [decision-trail.md#adr-019](decision-trail.md#adr-019).
6. ~~Are `package_instances` authoritative for all service assignments, and what remaining callers legitimately use `package_id/package_ids/stage_ids` on `tenants`?~~ **Decided 2026-09-10 (ADR-021):** `package_instances`/`stage_instances` ratified as authoritative. Three live call sites (`AddStageDialog.tsx` write, `GeneratedDocumentsTab.tsx` read, `AskVivScopeSelectorModal.tsx` read) still depend on the legacy columns and must migrate before those columns are vestigial. See [decision-trail.md#adr-021](decision-trail.md#adr-021).
7. ~~Must KPI cards be exact and real-time for the whole book, or is bounded freshness/approximation acceptable?~~ **Decided 2026-09-10 (ADR-022):** bounded freshness (server-side aggregate, refreshed on a short interval) approved as the default; sub-second whole-book exactness is not required unless a specific card is later named as an exception. See [decision-trail.md#adr-022](decision-trail.md#adr-022).
8. ~~What operational directory and context freshness SLOs are acceptable?~~ **Decided 2026-09-10 (ADR-023):** the paginated directory and tenant-detail views stay live/real-time by default post-redesign (no snapshot layer added preemptively); ≤300ms p95 ratified as the performance bar. A snapshot layer is authorized only if a measured scaling problem later proves the live query can't hit target. See [decision-trail.md#adr-023](decision-trail.md#adr-023).
9. ~~Which Ask Viv sources may be indexed, for how long, under what deletion SLA, and with what staff/client capability scopes?~~ **Decided 2026-09-10 (ADR-024):** sources broaden to the full operational entity set (packages, stages, tasks, action items, time entries, audits); retention tiered by tenant lifecycle (indefinite while active, 7-year-anchored clock after close, pending Angela's confirmation of that figure); deletion events propagate within ≤5 minutes; staff scope is operational/cross-tenant, client scope is strictly RTO-scoped to task management and consultant communication. See [decision-trail.md#adr-024](decision-trail.md#adr-024).
...
| P1+ implementation | P1.2-c repository source/configuration retirement is committed in `b771e3f7e`; hosted deletion and immediate read-only verification remain pending, with Carl's 60-minute observation waiver recorded; contact insertion, migration, and broader runtime implementation remain separately gated | [#1381](https://github.com/vivacityrto/unicorn-cms-f09c59e5/pull/1381) | [P1.1 membership/ownership compatibility scope](tenant-operating-model/p1/p1-1-membership-ownership-compatibility-scope.md) remains a planning draft; [P1.2-b](tenant-operating-model/p1/p1-2-b-ghost-contact-dry-run-execution-packet.md) records the completed QA snapshot; [P1.2-c](tenant-operating-model/p1/p1-2-c-activate-ghost-retirement-evidence.md) records the initial deletion, redeployment exception, and repository retirement; and [P1.2-d](tenant-operating-model/p1/p1-2-d-ghost-contact-apply-design-packet.md) records the planning-only apply shape |
