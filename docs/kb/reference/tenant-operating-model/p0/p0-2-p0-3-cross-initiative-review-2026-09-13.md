...
> **Status:** evidence crosswalk refreshed with a current read-only Realtime publication check; owner decisions remain open and this document approves no policy, capability row, fixture mutation, Realtime repair, or production action
> **Owner:** Tenant Operating Model with RBAC v6 and Client Health Activity Analytics
> **Inputs:** [TOM P0.2/P0.3 baseline packet](p0-2-p0-3-disposable-baseline-characterization.md), [cardinality/query-family follow-up](p0-2-p0-3-cardinality-query-family-follow-up-2026-09-13.md), [TOM P1.1 membership compatibility scope](../p1/p1-1-membership-ownership-compatibility-scope.md), [TOM first contact-promotion packet](../p1/p1-1-first-contact-promotion-implementation-packet.md), [RBAC P1-m golden preparation](../../rbac-v6/p1/p1-m-row-by-row-golden-preparation.md), [Client Health consultant evidence schema](../../client-health-activity-analytics/h0/h0-4-consultant-report-template-and-evidence-schema.md), [RBAC P1-l review draft](../../rbac-v6/p1/p1-l-aj-csc-golden-matrix-review-draft.md), [RBAC staff consumer inventory](../../rbac-v6/p1/p1-g-staff-internal-consumer-inventory.md), [Client Health plan](../../client-health-activity-analytics-plan-2026-09-03.md)
> **Evidence context:** current repository `origin/main@39fe0f5e`; the 2026-09-16 live publication read was captured directly from the hosted project; earlier aggregate reads in the linked follow-up were captured against `origin/main@2de9e154`; protected address follow-up is recorded in the [address audit entry](../../../../audit-log/entries/2026-09-13-tom-address-browser-characterization.md)
...
| Contact → invitation → acceptance | `TenantContactsSection` selects an explicit relationship role and calls the standard invitation path; the target packet treats acceptance as the boundary that materializes authenticated identity and membership | Promotion is a tenant-scoped action, not a capability implied by a contact label; accepted access still requires the RBAC action/scope contract | Pre-login contact and pending invitation are provenance states, not health/activity evidence; any downstream event needs source, actor, effective time, and freshness | First TOM implementation packet is drafted for review only; ghost projection, legacy activation retirement, and ledger cutover remain separate gates |
| Addresses, relationships, CSC assignments, connected tenants | QA has 3 synthetic addresses, 1 relationship, 2 assignments, and 2 connected tenants; production has 722 addresses, 2 relationships, 149 assignments, and 109 connected tenants; the protected address query was re-verified after the QA-only production-shape lookup/FK repair | Assignment/relationship semantics remain TOM-owned inputs to any capability scope | Health/triage must use resolved tenant identity and not infer ownership from missing rows | Relationship/assignment/connection reads are observed; production parity and Realtime remain separate owner gates |
...
| Realtime directory refresh | `ManageTenants.tsx` subscribes to `packages` and `tenant_csc_assignments`; a fresh 2026-09-16 read-only `pg_publication_tables` check confirms `supabase_realtime` includes only `tenant_conversations` and `tenant_messages` among the four checked tables | A listener is not an authorization decision; any future change needs explicit server/publication review | Realtime refresh must not manufacture freshness or health certainty | Current source/runtime mismatch is re-confirmed; no repair or publication change was performed |
| Ask Viv route, history, and generation | Staff route remains rollout-unavailable; dedicated Client User A run `34761219013` rendered one synthetic conversation with two turns and observed both client-history GETs at HTTP 200; no generation was attempted | Retrieval scope, exact capability, target tenant, and privacy/security review remain required | Missing/stale sources stay unavailable/unknown; LLM does not own totals, dates, risk, or permissions | Current client owner-scoped history read is characterized; staff/enabled-ring history, generation, provenance/freshness, and owner review remain open |
...
1. **TOM/RBAC/Client Health:** review the executed QA-only fixture expansion,
   the QA-only address-contract repair, and the observed Ask Viv owner-scoped
   history behavior.
