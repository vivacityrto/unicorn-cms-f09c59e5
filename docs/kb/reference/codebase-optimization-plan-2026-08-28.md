...
> **Phase 2.5 compliance-pack hook checkpoint (2026-09-06, PR pending):** `useCompliancePacks.tsx` narrowed five explicit-any findings to typed read rows and unknown-safe errors. The hook includes write-capable export creation/audit and Edge invocation paths; browser coverage must exercise only the initial read-only listing route. Baseline is 834 explicit-any / 841 total errors / 39 warnings across 193 files; merge remains gated on clean authenticated browser evidence.
