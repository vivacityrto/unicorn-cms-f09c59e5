...
| Phase 2.5 any-batch 5+ | Retire remaining `any` (3,679) in bounded batches — `src/hooks` (heaviest directory overall) may need splitting into several more batches; the `qc.ts`-adjacent QC components cluster and the rest of `src/hooks/academy` (153 findings) are reasonable next candidates | ⏳ Not started | — | — |
