...
| Phase 2.5 any-batch 8+ | Retire remaining `any` (~3,415, confirmed via committed `lint-baseline.json`) in bounded batches — `src/hooks` more broadly (heaviest directory overall, `useAcademyBuilderPickers.ts` and other non-EOS/non-enrollment academy hooks not yet swept) still needs splitting into several more batches; `src/components/academy/builder/**` also carries residual `any` findings surfaced but not fixed during 7b's fallout investigation (`LessonEditorPanel.tsx` alone had 6 remaining after the one required fix) | ⏳ Not started | — | — |
