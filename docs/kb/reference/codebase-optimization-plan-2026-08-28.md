...
> **Phase 2.5 client-audit portal checkpoint (2026-09-06, in progress):** Fresh importer sweep confirmed `useClientAuditReports` and `useClientActionPlanEnhanced` are active through `ClientAuditReportsSection`/`ClientHomePage`. Four explicit-any Supabase table/view boundaries were replaced with generated relation inference; report visibility, action-plan filtering, and tenant scoping are unchanged. Lint ratchet, frontend (298/15 skipped), Edge (260), and build passed; typecheck retains only documented baseline errors. Regenerated baseline is 763 explicit-any / 770 total errors / 39 warnings across 177 files. Authenticated SuperAdmin Playwright on `/client/home` passed 1/1 with zero page/console errors and no writes.
