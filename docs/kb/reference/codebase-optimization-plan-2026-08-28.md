...
| Phase 2.5 any-batch 9+ | Next candidate directory not yet surveyed — needs a fresh `npx eslint 'src/hooks/**/*.{ts,tsx}'` (or a broader `src/**`) ranking pass now that both `src/hooks/academy` and `src/hooks/useStage*` are fully closed out | ⏳ Not started | — | — |
