...
| Phase 2.5 hooks batch 5 | `react-hooks/exhaustive-deps` (4), `AttendancePanel.tsx` + `LiveMeetingView.tsx` — the plan's own "active workflow" flagged files, deferred from batch 4 | ⏳ PR open, not yet merged | [#533](https://github.com/vivacityrto/unicorn-cms-f09c59e5/pull/533) | 3 of 4 safe to fix directly (ref-guarded mutation calls, same pattern as batch 4); the 4th (phrase-draft seeding) was a real trap where the obvious "just add the missing dep" fix would let a user's deliberate field-clear get silently undone — suppressed instead. **Deliberately not verified via a live meeting session**: starting a real L10 meeting is a production write visible to the whole team, a different blast radius than a form dialog, so it wasn't done unprompted — flagged to Carl for a call on whether to test live before merging. Baseline dropped 165→161 warnings. Remaining `react-hooks/exhaustive-deps` (123) findings still queued. |
