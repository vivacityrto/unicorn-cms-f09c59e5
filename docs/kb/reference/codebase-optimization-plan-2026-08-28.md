...
### Phase 2.5 code-table contract checkpoint (2026-09-06, PR pending)

`useCodeTables.ts` and `codeTablesService.ts` remain active through
`CodeTablesAdmin`, `CodeRowDialog`, `CodeTableDataGrid`, `CreatableSelect`, and
app-settings flows. Fresh importer and generated RPC/table contract checks
confirmed the existing dynamic code-table operations. Nine explicit-any
findings were removed (4→0 and 5→0) using unknown-backed form values and
explicit Json boundaries; scalar UI keys are guarded without changing CRUD
behavior. `lint:ratchet`, frontend (298 passed / 15 skipped), Edge (260
passed), build, KB links (672/672), and typecheck (only established baseline
errors) pass under the shared mutex. Baseline is 839 explicit-any findings /
846 total errors / 39 warnings across 194 files. Write-capable RPCs were not
invoked; authenticated browser coverage remains pending.
