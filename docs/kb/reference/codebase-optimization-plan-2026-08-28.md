...
**Real gap found by this verification, not previously documented:** `src/pages/ProcessDetail.tsx` and `src/pages/ProcessForm.tsx` were part of the plan's own §5.3 inventory (26 "EOS and processes" files, routed at `/processes/:id`, `/processes/:id/edit`, `/processes/new` in `App.tsx` — still live, not dead code) but landed in neither PR #502 (EOS overview/read-only, 12 of its planned 14 files) nor PR #503 (EOS interactive/detail, exactly its planned 12 files, all `Eos*`). Both files still compose `<DashboardLayout>` directly across their multiple render branches, so those three routes still fully remount the staff shell on every navigation — the exact defect this program exists to fix. This was never caught because the child plan's own §12 "final documentation PR" step (regenerate the baseline, report remaining exceptions) was never done. Filing this as a small follow-up PR (2 files, same mechanical pattern as #502/#503) rather than fixing it inline here, per the plan's own change-size discipline (§14) — a docs-truth correction is not the place to also ship a code change.
...
The DashboardLayout direct-composition migration (Phase 2's own prerequisite exit gate for this phase, tracked in [`dashboard-direct-layout-migration-plan-2026-09-01.md`](dashboard-direct-layout-migration-plan-2026-09-01.md)) had its 19-PR core sequence land at PR [#518](https://github.com/vivacityrto/unicorn-cms-f09c59e5/pull/518), unblocking this phase per §8's ordering rule. A 2026-09-03 verification (see that document's §12) found this was 113/115 planned files, not 115/115 — `ProcessDetail.tsx`/`ProcessForm.tsx` were missed and still directly compose `DashboardLayout`. That 2-file gap is orthogonal to lint debt and doesn't retroactively invalidate starting this phase; it's tracked as its own follow-up.
...
| Phase 2.5 hooks batch 1 | `react-hooks/rules-of-hooks` (4) + `exhaustive-deps` (2), all in `BulkInvite.tsx` | ✅ Done | [#523](https://github.com/vivacityrto/unicorn-cms-f09c59e5/pull/523) | Genuine bug, not just lint debt: two early returns sat before 4 hook calls, throwing React's "Rendered more hooks" error on every real Super Admin page load once `profile` resolved. Fixed by moving the guard after all hooks (each hook already no-ops internally via `isSuperAdmin`); verified live via Playwright (SuperAdmin persona, zero console errors, correct data). Baseline dropped 3,748→3,744 errors, 191→189 warnings. Remaining `react-hooks/exhaustive-deps` (151) + `rules-of-hooks` (0) findings are still queued as further small, one-file/feature-at-a-time batches per the plan's caution around this risk tier. |
| Phase 2.5 PR N+ | Retire `any` (3,742, the large majority of the baseline) in bounded batches ordered by directory weight — `src/hooks` (1,234) is the heaviest single directory and may need splitting into 2–3 batches itself | ⏳ Not started | — | — |
