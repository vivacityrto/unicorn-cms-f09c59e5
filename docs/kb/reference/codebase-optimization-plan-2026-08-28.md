...
The three-seat council investigation is complete and recorded in [`dead-code-feature-consolidation-investigation-2026-09-04.md`](dead-code-feature-consolidation-investigation-2026-09-04.md). **Update 2026-09-04 (this PR):** three of the six highest-confidence cohorts from that register's §3.1 have since been implemented and merged — see the register's own cohort table for PR links (#570, #571, #574). This status paragraph previously said "no implementation PR has started," which was stale by the time of this correction. The remaining three highest-confidence cohorts (legacy suggestion pages, the orphaned document-version hook, the unused landing-page alias) and every second-wave/zero-inbound candidate are still unstarted. Each future implementation PR should still regenerate the import graph and route/metrics evidence from its own fresh branch-cut SHA before deleting — the register is evidence, not standing deletion authority, and per its own §2 caveat, `origin/main` continues to advance past any snapshot.
