...
### Phase 2.5 executive read-hook checkpoint (2026-09-06, PR pending)

`useExecutiveData.ts`, `useExecutiveHealth.ts`, and `useTenantNotes.ts` remain
active through `ExecutiveDashboard.tsx`, its executive subcomponents, and
`ManageTenants.tsx`. Fresh generated-view/RPC/table checks confirmed the
existing read-only contracts and realtime subscriptions. Nine explicit-any
findings were removed (3→0 per file) without changing filters, KPI
calculations, or subscription behavior. `lint:ratchet`, frontend (298 passed /
15 skipped), Edge (260 passed), build, KB links (672/672), and typecheck (only
the established `ClientLayout.tsx`/`useKpiSummary.tsx` baseline errors) pass
under the shared mutex. Baseline is 846 errors / 39 warnings across 193 files.
Authenticated SuperAdmin `/executive` Playwright remains the final gate; no
writes or new L10 bug were observed.
