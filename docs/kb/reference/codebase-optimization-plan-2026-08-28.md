...
| Phase 2.5 any-batch 6+ | Retire remaining `any` (3,641) in bounded batches — `src/hooks` (heaviest directory overall) may need splitting into several more batches; the rest of the EOS component cluster (`LiveMeetingView.tsx` 10, `ApplyTemplateDialog.tsx` 5, `useEosHealth.tsx`/`useEosSegueShares.tsx`/`useEosAgendaTemplates.tsx` 21 combined) and `src/hooks/academy` (153 findings) are reasonable next candidates | ⏳ Not started | — | — |
