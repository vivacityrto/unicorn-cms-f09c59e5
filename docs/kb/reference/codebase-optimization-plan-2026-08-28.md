...
### Phase 2.5 support-thread read checkpoint (2026-09-06, PR pending)

`useAdminHelpThreads.ts` is active from `SupportTicketsPage.tsx`,
`ClientMessagesPanel.tsx`, and admin thread detail. Fresh importer and
generated-table checks confirmed the existing `help_threads`, `help_messages`,
`users`, and tenant relation contracts. Five explicit-any findings were
removed (5→0) with bounded row types; no query, filter, or message-summary
behavior changed. `lint:ratchet`, frontend (298 passed / 15 skipped), Edge (260
passed), build, KB links (672/672), and typecheck (only established baseline
errors) pass under the shared mutex. Baseline is 843 explicit-any findings /
850 total errors / 39 warnings across 195 files. Authenticated
`/support-tickets` Playwright remains pending due the hosted-Supabase session
fetch-error blocker; no writes or new L10 bug were observed.
