...
> **Phase 2.5 KPI team read checkpoint (2026-09-06, PR pending):** `KpiTeamSection.tsx` narrowed five explicit-any findings to typed directory/profile rows and an unknown-safe RPC boundary. Static gates passed; baseline is 839 explicit-any / 846 total errors / 39 warnings across 194 files. Authenticated Playwright is pending because no SuperAdmin storage state is currently available; do not merge until a clean `/kpi` read-only pass is recorded.
