...
> **Phase 2.5 code-table boundary checkpoint (2026-09-06, in progress):** Fresh importer sweep confirmed `codeTablesService`/`useCodeTables` are active through CodeTablesAdmin, admin dialogs/grid, and CreatableSelect. Nine explicit-any findings were replaced with bounded scalar row values and Json RPC boundaries; CRUD RPC contracts and mutation behavior are unchanged. Lint ratchet, frontend (298/15 skipped), Edge (260), and build passed; typecheck retains only documented baseline errors. Regenerated baseline is 790 explicit-any / 797 total errors / 39 warnings across 183 files. Authenticated browser verification pending.
