...
| Phase 2.5 any-batch 9c+ | Continue `src/components/client` (208 findings remaining, 12 files: `ClientIntegrationsTab.tsx` 31, `ClientMessagesTab.tsx` 28, `RenewalConfirmDialog.tsx` 24, `AddTimeDialog.tsx` 19, `PackageDataManager.tsx` 18, `StartPackageDialog.tsx` 16, plus 5 files 5-6 each, then `ClientTimeTab.tsx` 71 — the single largest file in the cluster, likely its own sub-batch). Before selecting files, cross-check against the Phase 2.6 dead-code candidate register (path TBD — see coordination note) to avoid spending effort on a file already queued for retirement. | ⏳ Not started | — | — |
