...
| Phase 2.5 any-batch 7+ | Retire remaining `any` (~3,570) in bounded batches — `src/hooks/academy` (153 findings) is a reasonable next candidate now that the EOS cluster is fully closed out; `src/hooks` more broadly (heaviest directory overall) may still need splitting into several more batches | ⏳ Not started | — | — |
