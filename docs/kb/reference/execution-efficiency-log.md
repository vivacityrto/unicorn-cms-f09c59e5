...
| Final high-confidence frontend orphans | pending PR | 2026-09-04 | 186 | Removed orphaned `useDocumentVersions.tsx` (167 LOC), unused `Index.tsx` (18 LOC), and its unused lazy declaration (1 LOC). Metrics moved from 1,749 to 1,747 tracked product files and from 494,016 to 493,830 physical lines; typecheck, frontend/Edge tests, build, lint-ratchet, and unauthenticated Playwright all passed. Authenticated verification is not required because `/` and `/login` already render `Login` directly and neither deleted module had an inbound consumer. |
