...
- **2 genuinely missed, not a decision exception (fixed [#525](https://github.com/vivacityrto/unicorn-cms-f09c59e5/pull/525), same day):** `ProcessDetail.tsx` and `ProcessForm.tsx` (§5.3, part of the planned 26-file EOS/processes inventory, live-routed at `/processes/:id`, `/processes/:id/edit`, `/processes/new`) had landed in neither PR #502 nor #503 and still composed `DashboardLayout` directly. This was the one place the "115-file executable backlog... accounted for by orders 1–18" claim (§6) didn't hold — 113 of 115 executable files converted, not 115. Fixed as its own small follow-up PR (same mechanical plain-tier pattern as #502/#503, no `PermissionGate` needed) rather than as part of this retroactive documentation pass, per this plan's own §14-inherited discipline against mixing a docs correction with a code change. The 115-file backlog is now genuinely fully converted.

Not done, and still open follow-ups: route-composition conventions doc update (no new reusable guard-tier rule emerged beyond what individual PRs already documented inline), and bundle before/after measurement. Historical audit entries were not touched by this correction.
