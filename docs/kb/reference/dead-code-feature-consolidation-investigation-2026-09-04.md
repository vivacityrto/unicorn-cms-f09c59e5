...
| Audit route convergence | Characterization complete; implementation gated | `/audits` and `/audits/:id` use UUID `client_audits` via `AuditsAssessments`/`AuditWorkspaceNew`; `/audits/:id/findings`, `/actions`, and `/report` remain reachable legacy pages using numeric `audit`/`useAudits` contracts and `parseInt(id)`. Legacy `Audits`, `AuditWorkspace`, and `AuditWorkspacePlaceholder` are only unused lazy declarations in `App.tsx`. Next PR must add UUID-aware tab/deep-link convergence and authenticated refresh/permission/failure evidence before retiring the legacy island. |
