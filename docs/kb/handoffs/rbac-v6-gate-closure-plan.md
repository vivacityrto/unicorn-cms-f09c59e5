...
Every staff-facing listing surface (team directory pickers, meeting attendee auto-seeding, consultant/auditor/KPI-staff selectors) independently repeats some subset of `is_vivacity_internal = true AND archived = false AND disabled = false AND kpi_pod <> 'qa'` inline, in both Postgres functions (`get_vivacity_team_directory`, `get_vivacity_team_directory_staff`, `seed_meeting_attendees_from_roles`, `sync_l10_meeting_participants`, and others not yet audited) and ~8 frontend files querying `public.users` directly (`useAuditWorkspace.ts`, `KpiStaffSelector.tsx`, `NewAuditModal.tsx`, `ClientMessagesTab.tsx`, three Academy facilitator pickers, `StaffEngagementDetail.tsx`) — down from 9; `ReassignConsultantDialog.tsx` was confirmed dead code (zero repo-wide references, caller removed 2026-08-27) and retired outright in Phase 2.6 P6-B (2026-09-07) rather than migrated, per that packet's own instruction to remove dead predicates from this census instead of consolidating them. This is the same "logic duplicated instead of centralized" problem the rest of this doc identifies for permissions generally, just for "is this a listable human" instead of "can this user do X":
