...
```
Classic PL/pgSQL bug: a `RECORD`-typed variable is referenced before a `SELECT INTO` populates it (likely the `SELECT INTO v_last_job ... ` returned zero rows and the function doesn't guard for that before accessing a field on `v_last_job`). This is **separate** from the previously-known outage in `tga_sync_broken_l3_gate` (the `l3_gate_tga_sync_cluster` auth.uid()-vs-service-role issue) — that memory is about the *sync* RPC; this is a bug in the *status* RPC. Worth checking whether both are still live or if one has been superseded.
...
Both are called out per `feedback_verify_against_origin_main`-style discipline — better to flag an ambiguous lead than report a false positive.
...
- `tga_sync_broken_l3_gate`, `auth_uid_gated_rpc_service_role_antipattern` — related but distinct TGA issue
