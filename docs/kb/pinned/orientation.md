...
3. **Lovable owns schema in practice; hand-written changes are branch+PR, never direct-to-main.** Lovable users (Angela, Dave, Khian — see `team-roles.md`) still push schema migrations alongside frontend code direct to `main` with no pre-merge review or sign-off gate — that part of ADR-011's operating model hasn't changed. The three-step RLS ritual (item 2 above) is the *correct* technical pattern, but enforcement there is post-merge. **This is no longer symmetric for hand-written code as of the current `AGENTS.md`:** a direct git hotfix (Claude Code or otherwise) is a branch, opened as a PR, and the *only* path to `main` is merging that PR — never a direct push, and merging always requires a fresh, explicit in-session ask (a standing "yes" from an earlier session doesn't carry forward). See `reference/decision-trail.md → ADR-011` for the original rationale (preserved as historical) and the amending ADR (§ below / decision-trail.md) for what superseded the "hand-written code also has no gate" framing.
4. **Edge functions authenticate the caller via the shared helper, not a hand-rolled check.** Use `requireCaller`/`requireSharedSecret`/`requireInternalEmailSecret` from `supabase/functions/_shared/` (or the cron-secret gate for machine-to-machine calls) before any DB read/write branch — not a one-off `supabase.auth.getUser(callerToken)` + manual `unicorn_role`/`tenant_id` check. The service-role key bypasses RLS, so the function is the only enforcement point; see `AGENTS.md → Edge Function security guardrails` for the full checklist (every mode/branch of a multi-action function must share the same gate, runtime allowlists for any union-typed role/action field, etc.).
