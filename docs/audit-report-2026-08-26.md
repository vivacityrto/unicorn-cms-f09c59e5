...
- **Resolution (2026-08-26; retirement follow-up 2026-08-28):** The orphaned, half-built page implementation and its four sole-consumer child components were removed, along with the `client_id` profile field/select used only by that page. The `/client/eos` route is also unregistered; the supported client portal is `/client/home`. The `get_client_eos_overview` Postgres RPC remains in place and unreferenced by the frontend; dropping it is a schema change requiring its own migration/audit-log entry.
...
| F-012 | Retired the old `/client/eos` page, sole-consumer components, and route. The supported client portal is `/client/home`. | Focused route/source checks cover the absence of the retired route. `get_client_eos_overview` remains in the DB and unreferenced; dropping it needs its own migration/audit entry if wanted later. |
