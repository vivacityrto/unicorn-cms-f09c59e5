...
- **Resolution (2026-08-26):** On review, `/client/eos` was orphaned, half-built work — shelled in the internal-staff `DashboardLayout` rather than `ClientLayout` like every other `/client/*` page, and not linked from any nav/menu in the codebase. Carl confirmed it should be deleted rather than fixed. Removed the route (`src/App.tsx`), the page (`src/pages/ClientEosOverview.tsx`), its four sole-consumer child components (`src/components/eos/client/Client{Rocks,Issues,Headlines,Summaries}List.tsx`), and the `client_id` profile field/select added to `useAuth.tsx` solely to feed it (no other caller reads `profile.client_id`). The `get_client_eos_overview` Postgres RPC is now unreferenced by the frontend but was left in place — dropping it is a schema change requiring its own migration/audit-log entry and is out of scope here unless requested.
...
- **Resolution (2026-08-26):** Before drafting the transactional RPC, checked whether `PackageDetail.tsx` (and its `/package/:id` route) is actually reachable — it isn't. No nav link anywhere in the codebase references `/package/:id`; its own wrapper (`PackageDetailWrapper.tsx`) is itself never imported by `App.tsx`. Worse, the reorder handler this finding cites doesn't even target real data: it reorders `stages` state built from the legacy `tenants.stage_ids` array (only 6 tenants in production still have it populated) joined against the master `stages` catalog, then tries to persist via `.from('package_stages').update({ order_number: ... })` — but `package_stages`'s real column is `sort_order`, not `order_number`, and the row id used is `stages.id`, not `package_stages.id`. Every reorder attempt would fail outright with a column-not-found error, not silently partially-succeed. Confirmed via `query_logs` that no such error has actually fired in the last 24h (nobody's hit this dead page). Carl confirmed this is abandoned work; resolved by deleting `PackageDetail.tsx`, `PackageDetailWrapper.tsx`, and the `/package/:id` route rather than building a migration for it — see [F-012](#f-012--client-eos-overview-uses-an-unpopulated-auth-field) for the same treatment applied to `/client/eos` this same session.
...
| F-012 | Deleted `/client/eos` entirely (route, page, four sole-consumer child components) and reverted the `client_id` profile field/select that existed only to feed it. On review this was orphaned, half-built work — shelled in the internal-staff `DashboardLayout` rather than `ClientLayout` like every other `/client/*` page, and linked from no nav/menu anywhere in the codebase. Carl confirmed abandonment; resolved by removal, not fix. | `npx tsc --noEmit` passes after removal. **Live-verified:** `/client/eos` now 404s. `get_client_eos_overview` RPC left in the DB, now unreferenced by the frontend — dropping it needs its own migration/audit entry if wanted later. |
