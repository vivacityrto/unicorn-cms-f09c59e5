...
- ~~`generate-recovery-link` and `invite-to-tenant`: add CORS/role controls, stop logging links or secrets, and validate recipient/role allowlists.~~ **RESOLVED — `invite-to-tenant` fixed and deployed** (2026-08-18, PR #342, `docs/audit-log/entries/2026-08-18-invite-to-tenant-role-allowlist.md`, deployed as v92) — added the same `CLIENT_ROLES` allowlist `invite-user` uses (the tenant-admin auth path had no ceiling on `role`, so a tenant Admin could request `role: "Super Admin"`), fixed `corsHeaders` being used as a static object instead of called with `(req)`, stopped logging the plaintext invite link/token. `generate-recovery-link` re-verified as already compliant — no changes needed.
- ~~The DB trigger meant to backstop the above (`enforce_invitation_role_ceiling`) unconditionally exempted `service_role`-authenticated inserts, which is how every edge function writes to `user_invitations` — so it provided no real protection for `invite-user`/`activate-ghost-user` beyond their own app-layer `admin.team_users.manage` check.~~ **RESOLVED, migration applied to production 2026-08-18** (`docs/audit-log/entries/2026-08-18-invitation-role-ceiling-service-role-bypass.md`, migration `20260818040717_fix_invitation_role_ceiling_service_role_bypass`) — the trigger now uses `NEW.invited_by` (stamped by each edge function's own verified caller lookup) as the acting-user identity under `service_role`, instead of exempting the write outright. Confirmed no behavior change for any current legitimate caller (`admin.team_users.manage` is currently granted at `full` only to Super Admin).
