...
1. ~~**`import-unicorn1-client` destructive ordering (N4).**~~ **Fixed in source, not yet deployed** (2026-08-18, see `docs/audit-log/entries/2026-08-18-import-unicorn1-destructive-ordering-and-clickup-sync-gate.md`). The Unicorn 1 client-existence check now runs first and unconditionally, before `clearTenantInstanceData`; the destructive clear is recorded in `client_audit_log`. Regression test added. PR pending review; production still runs the pre-fix version until deployed with explicit authorization.
2. ~~**Unauthenticated service-role entrypoints (N2/N3).**~~ **`sync-clickup-tasks` fixed in source, not yet deployed** (same 2026-08-18 entry) — gated on `requireCaller(req, "admin.team_users.manage", "full")` plus request-aware CORS. `import-clickup-csv` re-verified as already gated (2026-08-16 hardening); no changes needed there. PR pending review; production still runs the pre-fix `sync-clickup-tasks` version until deployed with explicit authorization.
...
4. **Duplicate helper implementations (N10/M5/L8).** Re-checked 2026-08-18: no duplicate `requireCaller*`/`auth-helpers*` files found anywhere under `supabase/functions/**` (searched both `_shared/` and every per-function directory) — appears already consolidated to the single `_shared/requireCaller.ts`. Still missing: CI that rejects duplicate helper filenames going forward (not added). Verify this holds before trusting it long-term; a future function could still hand-roll its own copy.
