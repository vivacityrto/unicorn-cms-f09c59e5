...
| **Current** | **1,850** | **505,102** |
| **Removed** | **−178 (−8.8%)** | **−33,298 (−6.2%)** |
...
All PRs above are merged; `main` matches the "Current" row exactly. **8 of the original 19 §3 items are now resolved** (3 frontend: `StageCompletenessWidget.tsx`, `StageHealthPanel.tsx`, `useDocumentAIConfidence.tsx`; 5 edge functions: `assign-package-to-tenant`, `academy-fetch-vimeo-showcase`, `validate-ai-assist`, `generate-audit-report`, `mailgun-webhooks`) — and all 6 retired functions (the 5 above + `assign-package-to-tenant`) are now confirmed fully deleted from the live Supabase project too (verified via `list_edge_functions`), not just stubbed. The remaining 11 (1 frontend: `microsoft/scopes.ts`; 10 edge functions) are deliberately left for Carl or external checks — see §3/§7 for the full list.
