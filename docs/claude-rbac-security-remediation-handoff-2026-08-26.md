...
| F-012 | `src/App.tsx`, `src/pages/ClientEosOverview.tsx` (deleted), `src/components/eos/client/*` (deleted), `src/hooks/useAuth.tsx` | `/client/eos` was orphaned/half-built (staff `DashboardLayout` shell, no nav link anywhere) — confirmed abandoned and deleted outright, along with the `client_id` profile field/select added only to feed it. | Resolved by removal, not fix. `npx tsc --noEmit` passes. No Demo RTO verification needed — route no longer exists. |
