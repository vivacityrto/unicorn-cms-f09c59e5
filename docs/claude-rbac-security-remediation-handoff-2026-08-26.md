...
3. **Use real client personas in isolated browser contexts.** Ask the user to sign into Demo RTO in a fresh persistent/browser context. Do not rely on SuperAdmin "View as Client". Verify RBAC deep links, the supported `/client/home` portal, document-request dialog from Client Home/TGA, and a linked-audit stage link. Do not verify retired `/client/eos`. Use only read-only flows unless an explicitly safe disposable test tenant and cleanup plan are available.
...
2) ask me to authenticate Demo RTO in a fresh browser context, then run read-only Playwright checks for real client deep links, Client Home/document request dialog, and linked-audit navigation;
