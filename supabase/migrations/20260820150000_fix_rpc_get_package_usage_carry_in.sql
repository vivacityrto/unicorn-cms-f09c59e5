-- rpc_get_package_usage() backs the "Package Burn-down" widget on the Client
-- Overview tab (ClientTimeSummaryCard.tsx, via usePackageUsageQuery.tsx) - a
-- fifth, independent implementation of "package usage" alongside
-- fn_package_used_minutes(), v_package_burndown, and
-- get_client_package_dashboard(). It already excludes carry_over entries
-- from "used" and windows by renewal year (fixed 2026-07-14), but:
--   1. Never included package_renewal_periods.carried_in_minutes in its
--      included_minutes total - same missing-carry-in bug as everywhere
--      else, just not caught until Carl spotted "13:53 / 28:00" on Southern
--      Education's Overview tab still missing the +13:02 carried over.
--   2. Derived its renewal-year window as (next_renewal_date - 1yr) to
--      next_renewal_date, instead of reading start_renewal_date directly -
--      functionally the same for well-behaved data, but inconsistent with
--      the anchored, non-derived source every other fixed query now uses.
