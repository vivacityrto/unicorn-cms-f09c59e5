...
BEGIN
  ...
  -- Check feature flag
  SELECT COALESCE(governance_use_stage_documents, false)
    INTO v_use_stage_documents
    FROM app_settings
   LIMIT 1;
  ...
  LOOP
    ...
    -- 2d. Documents — feature-flagged path
    IF v_use_stage_documents THEN
      -- NEW PATH: Seed from stage_documents mapping table
      ...
      SELECT sd.document_id, v_stage_instance_id, p_tenant_id, 'pending', false,
       ...
       WHERE sd.stage_id = v_stage.stage_id::integer
         AND sd.is_active = true;
    ELSE
      -- LEGACY PATH: Seed from documents table directly
  ...
  INSERT INTO client_audit_log (
    ...
    jsonb_build_object(
      ...
      'document_source', CASE WHEN v_use_stage_documents THEN 'stage_documents' ELSE 'documents.stage' END
    )
  );
