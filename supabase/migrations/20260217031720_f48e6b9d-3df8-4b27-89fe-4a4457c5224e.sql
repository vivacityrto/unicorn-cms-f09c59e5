...
create or replace function public.calculate_attention_score(
  ...
  p_renewal integer,
  p_burn integer,
  p_overdue_tasks integer default 0
)
returns integer
language sql
immutable
...
as $$
  ...
  select
    case
      when p_overdue_tasks >= 3 and score < 70 then 70
      else score
    end
  from base;
$$;

-- Phase 2+3: Rebuild attention view
  ...
  select
    ...
    least(100, greatest(0,
      case b.worst_stage_health_status
        when 'critical' then 100 when 'at_risk' then 70 when 'monitoring' then 35 else 0
    ...
    least(100,
      case
        when b.days_since_activity <= 7 then 0 when b.days_since_activity <= 14 then 25
        when b.days_since_activity <= 21 then 50 when b.days_since_activity <= 30 then 75 else 100
      end + case when b.open_tasks_count > 0 and b.days_since_activity >= 15 then 10 else 0 end
    ) as staleness_score,
    case
      when b.days_to_renewal is null then 0 when b.days_to_renewal <= 0 then 100
      when b.days_to_renewal <= 14 then 100 when b.days_to_renewal <= 30 then 75
      when b.days_to_renewal <= 60 then 50 when b.days_to_renewal <= 90 then 25 else 0
...
final as (
  select
    s.*,
    -- Phase 2: pass overdue_tasks as 8th param for floor logic
    calculate_attention_score(
      s.stage_score, s.gaps_score, s.risk_score, s.staleness_score,
      s.task_score, s.renewal_score, s.burn_score,
      s.compliance_overdue_tasks
    ) as attention_score,
    -- Phase 3: Critical Overdue driver placed first in array
    (select jsonb_agg(sub.d order by (sub.d->>'impact')::integer desc)
     from (
       select d.value as d
       from jsonb_array_elements(jsonb_build_array(
         case when s.compliance_overdue_tasks >= 3
           then jsonb_build_object('driver','Critical Overdue Compliance',
                  'impact', 70, 'value', s.compliance_overdue_tasks || ' overdue tasks')
           else null end,
         ...
         jsonb_build_object('driver','Renewal pressure',
           'value', case when s.days_to_renewal is not null
                         then s.days_to_renewal || ' days to renewal' else 'No renewal date' end,
       ...
       where d.value is not null and d.value != 'null'::jsonb
         and (d.value->>'impact')::integer > 0
       limit 3
     ) sub
