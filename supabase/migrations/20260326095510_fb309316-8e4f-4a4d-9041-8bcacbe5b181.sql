...
-- Drop the stale bigint overload to prevent "multiple candidate functions" errors
DROP FUNCTION IF EXISTS public.publish_stage_version(bigint, text);

-- Recreate with integer parameter and fix client_task_instances.status type (integer, not text)
...
BEGIN
  ...
  -- 6. Snapshot emails directly from emails table (no join needed)
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
  ...
  -- 7. Snapshot documents (unchanged - already correct)
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
  ...
  LOOP
    ...
    -- Insert missing client_task_instances (status is integer, not text)
