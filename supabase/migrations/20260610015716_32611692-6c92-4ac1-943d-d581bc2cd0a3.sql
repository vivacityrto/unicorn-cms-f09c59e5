...
BEGIN
  -- Resolve package metadata (now includes slug for billing classification)
  ...
  -- Derive billing_type / billing_category from package name + slug.
  -- Matches the Feb 2026 backfill (migration 20260216005525) exactly,
  -- including the -bc CRICOS suffix.
  IF v_pkg_name LIKE 'KS%' OR v_pkg_name LIKE 'KickStart%' OR v_pkg_slug LIKE '%ks%' THEN
    v_billing_type := 'non_billable';
    v_billing_category := NULL;
  ELSIF v_pkg_name LIKE 'M-GTO%' THEN
    v_billing_type := 'billable';
    v_billing_category := 'other';
  ELSIF v_pkg_name LIKE 'M-%' AND (
    ...
    v_billing_type := 'billable';
    v_billing_category := 'membership_cricos';
  ELSIF v_pkg_name LIKE 'M-%' THEN
    v_billing_type := 'billable';
    v_billing_category := 'membership_rto';
  ELSE
    v_billing_type := 'billable';
    v_billing_category := 'other';
  END IF;

  -- Duplicate-type guard (unchanged)
  ...
  -- 1. Insert into package_instances (now with billing_type + billing_category)
  ...
  -- 3. Audit log (now records derived billing classification)
