...
BEGIN
  ...
  LOOP
    ...
    SELECT d.id, v_sii, NEW.tenant_id, 'pending', false
      FROM public.documents d WHERE d.stage = v_stage.stage_id::integer;
  END LOOP;
...
DROP TRIGGER IF EXISTS trg_seed_stage_instances ON public.package_instances;
CREATE TRIGGER trg_seed_stage_instances
AFTER INSERT ON public.package_instances
FOR EACH ROW
EXECUTE FUNCTION public.seed_stage_instances_from_template();
...
REVOKE EXECUTE ON FUNCTION public.start_client_package(bigint, bigint, uuid) FROM anon, PUBLIC;
GRANT EXECUTE ON FUNCTION public.start_client_package(bigint, bigint, uuid) TO authenticated;