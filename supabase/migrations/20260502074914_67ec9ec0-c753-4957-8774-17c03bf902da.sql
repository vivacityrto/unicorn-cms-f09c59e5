...
WITH stage_agg AS (
  ...
  WHERE cai.package_id IS NOT NULL AND cai.completed_at IS NULL AND (COALESCE(cai.status, 'open'::text) <> ALL (ARRAY['completed'::text, 'cancelled'::text]))
  ...
  WHERE COALESCE(cti.is_archived, false) = false AND cti.completion_date IS NULL AND COALESCE(cti.status, 0) <> 2 AND COALESCE(si.released_client_tasks, false) = true
  GROUP BY si.packageinstance_id
), tasks_agg AS (
  ...
  WHERE te.package_instance_id IS NOT NULL AND te.duration_minutes IS NOT NULL AND te.duration_minutes > 0 AND (pi2.start_date IS NULL OR te.start_at >= pi2.start_date)
...
SELECT pi.id AS package_instance_id,
  ...
  GREATEST(COALESCE(na.notes_last_updated, '1970-01-01 00:00:00+00'::timestamp with time zone), COALESCE(sa.stage_last_updated, '1970-01-01 00:00:00+00'::timestamp with time zone), COALESCE(ta.tasks_last_updated, '1970-01-01 00:00:00+00'::timestamp with time zone), COALESCE(ha.max_te_at, '1970-01-01 00:00:00+00'::timestamp with time zone)) AS last_activity_at,
  ...
  CASE
    ...
    WHEN GREATEST(COALESCE(na.notes_last_updated, '1970-01-01 00:00:00+00'::timestamp with time zone), COALESCE(sa.stage_last_updated, '1970-01-01 00:00:00+00'::timestamp with time zone), COALESCE(ta.tasks_last_updated, '1970-01-01 00:00:00+00'::timestamp with time zone), COALESCE(ha.max_te_at, '1970-01-01 00:00:00+00'::timestamp with time zone)) < (now() - '30 days'::interval) OR (COALESCE(p.total_hours, 0) + COALESCE(pi.hours_added, 0)) > 0 AND (COALESCE(ha.hours_used_calc, 0::numeric) / (COALESCE(p.total_hours, 0) + COALESCE(pi.hours_added, 0))::numeric) >= 0.95 THEN 'stuck'::text
    WHEN GREATEST(COALESCE(na.notes_last_updated, '1970-01-01 00:00:00+00'::timestamp with time zone), COALESCE(sa.stage_last_updated, '1970-01-01 00:00:00+00'::timestamp with time zone), COALESCE(ta.tasks_last_updated, '1970-01-01 00:00:00+00'::timestamp with time zone), COALESCE(ha.max_te_at, '1970-01-01 00:00:00+00'::timestamp with time zone)) < (now() - '14 days'::interval) OR (COALESCE(p.total_hours, 0) + COALESCE(pi.hours_added, 0)) > 0 AND (COALESCE(ha.hours_used_calc, 0::numeric) / (COALESCE(p.total_hours, 0) + COALESCE(pi.hours_added, 0))::numeric) >= 0.75 OR COALESCE(ta.overdue_tasks, 0) > 0 THEN 'drifting'::text
    ELSE 'on_track'::text
  END AS status_pill,
