...
-- Pre-deploy verification (must return 0 rows):
...
-- Verified: 0 orphan rows at time of authoring.

-- Change A: drop legacy package_id write from rpc_publish_stage_tasks.
...
BEGIN
  ...
  SELECT si.stage_id, si.packageinstance_id, pi.tenant_id
    INTO v_stage_id, v_pkg_inst_id, v_tenant_id
...
-- Change B: enforce FK on client_action_items.package_id -> packages(id).
