-- Track which follow-up job a skipped item was requeued into, so the
-- "Review skipped" UI can warn staff before they create a second, duplicate
-- follow-up job for items someone already requeued.
ALTER TABLE public.bulk_document_job_items
  ADD COLUMN IF NOT EXISTS requeued_to_job_id uuid REFERENCES public.bulk_document_jobs(id);

CREATE INDEX IF NOT EXISTS idx_bulk_document_job_items_requeued_to_job_id
  ON public.bulk_document_job_items(requeued_to_job_id)
  WHERE requeued_to_job_id IS NOT NULL;
...
BEGIN
  ...
  -- Mark the source skipped items as handled by this follow-up job so the
  -- "Review skipped" UI can warn staff before they create a second,
  -- duplicate follow-up job for the same items.
  UPDATE public.bulk_document_job_items
  SET requeued_to_job_id = v_job_id
