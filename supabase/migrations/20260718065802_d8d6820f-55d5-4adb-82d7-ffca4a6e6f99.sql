-- Restrictive RLS backstops for tables where permissive policies alone
-- are insufficient defense-in-depth against overly-broad grants or future
-- permissive policy drift.
--
-- engagement_exit_interviews WITH CHECK mirrors USING so linked offboarding
-- users can still upsert their own interview (permissive linked_user_*
-- policies), while admins retain full write access. (`or true` would leave
-- INSERT/UPDATE unrestricted by this backstop.)

BEGIN;

-- Linked offboarding users must be able to SELECT their own engagement so
-- MyExitInterview works and so EXISTS() checks in engagement_exit_interviews
-- policies can evaluate under RLS.
...
TO authenticated
USING (
  linked_unicorn_user_id = (SELECT auth.uid())
...
USING (
  ...
  OR linked_unicorn_user_id = (SELECT auth.uid())
)
...
ON public.engagement_exit_interviews
AS RESTRICTIVE
FOR ALL
TO authenticated
USING (
  ...
  OR EXISTS (
    SELECT 1
    FROM public.staff_engagements se
    WHERE se.id = engagement_exit_interviews.engagement_id
      AND se.linked_unicorn_user_id = (SELECT auth.uid())
