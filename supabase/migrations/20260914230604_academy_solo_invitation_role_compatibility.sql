-- Academy Solo invitations use the existing legacy `User` role vocabulary.
-- Academy-only authorization remains authoritative through
-- tenant_users.relationship_role = 'academy_user' and access_scope =
-- 'academy_only'. This keeps the invitation acceptance path compatible with
-- public.dd_unicorn_roles without coupling Solo accounts to RTO packages.
