-- display_name now derives from first_name + last_name only; full_name is the Full Legal Name field and is not used for display.
...
COMMENT ON VIEW public.v_client_tenant_users IS
  'Client portal tenant user list. display_name is derived from first_name + last_name (fallback email). full_name (Full Legal Name) is intentionally not used for display.';

-- Idempotent data record for audit trail (no-op in current prod state).
UPDATE public.users
SET full_name = TRIM(COALESCE(first_name, '')) || ' ' || TRIM(COALESCE(last_name, ''))
WHERE email = 'hilton@hpatraining.com.au'
  AND full_name IS DISTINCT FROM TRIM(COALESCE(first_name, '')) || ' ' || TRIM(COALESCE(last_name, ''));