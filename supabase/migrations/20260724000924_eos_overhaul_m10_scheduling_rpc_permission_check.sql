-- ============================================================
-- EOS Meeting Overhaul — Migration 10 (scheduling RPC permission check)
...
-- project convention.
--
-- Gap found by Cursor Bugbot review on PR #39 (round 2), not anticipated
-- in the original plan: create_meeting_from_configuration (M8),
-- sync_meeting_to_configuration and skip_meeting_occurrence (M6) only
-- check is_vivacity_team_safe() - "is any active internal Vivacity staff
-- member" - not the eos.meetings.l10.create feature permission the
-- frontend actually gates the Schedule/Sync/Skip buttons on. Confirmed
-- live: is_vivacity_team_safe() has no role-level distinction at all, so
-- any active internal staff member (including roles the UI hides these
-- actions from, e.g. BGT/CET/CSC) could call these RPCs directly and
-- schedule/sync/skip meetings, bypassing the UI-only gate entirely.
--
-- Re-CREATE OR REPLACE each function verbatim, adding a
-- has_permission('eos.meetings.l10.create') check alongside the existing
-- staff check - never amending the already-committed M6/M8/M9 migrations
-- that originally defined them, per this branch's established discipline.
-- Bodies below are byte-for-byte the current (M9-superseded, for the two
-- that M9 touched) live definitions plus the one added check each.
-- ============================================================
