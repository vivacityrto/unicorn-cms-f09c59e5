...
-- Add is_core flag to staff_tasks (template level)
ALTER TABLE public.staff_tasks
  ADD COLUMN IF NOT EXISTS is_core boolean NOT NULL DEFAULT true;

-- Add is_core flag to staff_task_instances (runtime level)
ALTER TABLE public.staff_task_instances
  ADD COLUMN IF NOT EXISTS is_core boolean NOT NULL DEFAULT true;

-- Update start_client_package RPC to copy is_core from template
CREATE OR REPLACE FUNCTION public.start_client_package(
  p_tenant_id integer,
  p_package_id integer,
  p_assigned_csc_user_id text DEFAULT NULL
)
RETURNS bigint
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    -- 2a. Staff tasks (now copies is_core from template)
    INSERT INTO staff_task_instances (stafftask_id, stageinstance_id, status_id, status, is_core)
    SELECT st.id, v_stage_instance_id, 0, 'Not Started', st.is_core
      FROM staff_tasks st
