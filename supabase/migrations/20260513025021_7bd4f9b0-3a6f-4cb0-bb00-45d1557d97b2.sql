...
CREATE POLICY client_action_item_comments_tenant_all ON public.client_action_item_comments AS PERMISSIVE FOR ALL TO public USING ((tenant_id IN ( SELECT tenant_users.tenant_id FROM tenant_users WHERE (tenant_users.user_id = (SELECT auth.uid())))));

-- client_action_items
DROP POLICY IF EXISTS client_action_items_tenant_delete ON public.client_action_items;
CREATE POLICY client_action_items_tenant_delete ON public.client_action_items AS PERMISSIVE FOR DELETE TO public USING (((EXISTS ( SELECT 1 FROM users u WHERE ((u.user_uuid = (SELECT auth.uid())) AND (u.tenant_id = client_action_items.tenant_id)))) OR (EXISTS ( SELECT 1 FROM users u WHERE ((u.user_uuid = (SELECT auth.uid())) AND (u.unicorn_role = ANY (ARRAY['Super Admin'::unicorn_role, 'Team Leader'::unicorn_role])))))));

DROP POLICY IF EXISTS client_action_items_tenant_insert ON public.client_action_items;
CREATE POLICY client_action_items_tenant_insert ON public.client_action_items AS PERMISSIVE FOR INSERT TO public WITH CHECK (((EXISTS ( SELECT 1 FROM users u WHERE ((u.user_uuid = (SELECT auth.uid())) AND (u.tenant_id = client_action_items.tenant_id)))) OR (EXISTS ( SELECT 1 FROM users u WHERE ((u.user_uuid = (SELECT auth.uid())) AND (u.unicorn_role = ANY (ARRAY['Super Admin'::unicorn_role, 'Team Leader'::unicorn_role])))))));

DROP POLICY IF EXISTS client_action_items_tenant_select ON public.client_action_items;
CREATE POLICY client_action_items_tenant_select ON public.client_action_items AS PERMISSIVE FOR SELECT TO public USING (((EXISTS ( SELECT 1 FROM users u WHERE ((u.user_uuid = (SELECT auth.uid())) AND (u.tenant_id = client_action_items.tenant_id)))) OR (EXISTS ( SELECT 1 FROM users u WHERE ((u.user_uuid = (SELECT auth.uid())) AND (u.unicorn_role = ANY (ARRAY['Super Admin'::unicorn_role, 'Team Leader'::unicorn_role])))))));

DROP POLICY IF EXISTS client_action_items_tenant_update ON public.client_action_items;
CREATE POLICY client_action_items_tenant_update ON public.client_action_items AS PERMISSIVE FOR UPDATE TO public USING (((EXISTS ( SELECT 1 FROM users u WHERE ((u.user_uuid = (SELECT auth.uid())) AND (u.tenant_id = client_action_items.tenant_id)))) OR (EXISTS ( SELECT 1 FROM users u WHERE ((u.user_uuid = (SELECT auth.uid())) AND (u.unicorn_role = ANY (ARRAY['Super Admin'::unicorn_role, 'Team Leader'::unicorn_role])))))));

-- client_ai_messages
DROP POLICY IF EXISTS client_own_messages_select ON public.client_ai_messages;
CREATE POLICY client_own_messages_select ON public.client_ai_messages AS PERMISSIVE FOR SELECT TO public USING ((EXISTS ( SELECT 1 FROM client_ai_sessions s WHERE ((s.id = client_ai_messages.session_id) AND (s.user_id = (SELECT auth.uid()))))));
...
CREATE POLICY client_alerts_insert_rpc ON public.client_alerts AS PERMISSIVE FOR INSERT TO public WITH CHECK ((tenant_id IN ( SELECT connected_tenants.tenant_id FROM connected_tenants WHERE (connected_tenants.user_uuid = (SELECT auth.uid())))));

DROP POLICY IF EXISTS client_alerts_select_tenant ON public.client_alerts;
CREATE POLICY client_alerts_select_tenant ON public.client_alerts AS PERMISSIVE FOR SELECT TO public USING ((tenant_id IN ( SELECT connected_tenants.tenant_id FROM connected_tenants WHERE (connected_tenants.user_uuid = (SELECT auth.uid())))));

DROP POLICY IF EXISTS client_alerts_update_dismiss ON public.client_alerts;
CREATE POLICY client_alerts_update_dismiss ON public.client_alerts AS PERMISSIVE FOR UPDATE TO public USING (((tenant_id IN ( SELECT connected_tenants.tenant_id FROM connected_tenants WHERE (connected_tenants.user_uuid = (SELECT auth.uid())))) AND (EXISTS ( SELECT 1 FROM users WHERE ((users.user_uuid = (SELECT auth.uid())) AND (users.role = ANY (ARRAY['SuperAdmin'::text, 'Admin'::text])))))));
...
CREATE POLICY client_audit_log_superadmin_insert ON public.client_audit_log AS PERMISSIVE FOR INSERT TO public WITH CHECK ((EXISTS ( SELECT 1 FROM users WHERE ((users.user_uuid = (SELECT auth.uid())) AND (users.unicorn_role = ANY (ARRAY['Super Admin'::unicorn_role, 'Team Leader'::unicorn_role]))))));

DROP POLICY IF EXISTS client_audit_log_superadmin_select ON public.client_audit_log;
CREATE POLICY client_audit_log_superadmin_select ON public.client_audit_log AS PERMISSIVE FOR SELECT TO public USING ((EXISTS ( SELECT 1 FROM users WHERE ((users.user_uuid = (SELECT auth.uid())) AND (users.unicorn_role = ANY (ARRAY['Super Admin'::unicorn_role, 'Team Leader'::unicorn_role]))))));

-- client_audit_references
DROP POLICY IF EXISTS audit_refs_staff_all ON public.client_audit_references;
CREATE POLICY audit_refs_staff_all ON public.client_audit_references AS PERMISSIVE FOR ALL TO public USING ((EXISTS ( SELECT 1 FROM users u WHERE ((u.user_uuid = (SELECT auth.uid())) AND ((u.is_vivacity_internal = true) OR (u.global_role = ANY (ARRAY['superadmin'::text, 'admin'::text])))))));

DROP POLICY IF EXISTS audit_refs_tenant_read ON public.client_audit_references;
CREATE POLICY audit_refs_tenant_read ON public.client_audit_references AS PERMISSIVE FOR SELECT TO public USING ((EXISTS ( SELECT 1 FROM tenant_members tm WHERE ((tm.tenant_id = client_audit_references.subject_tenant_id) AND (tm.user_id = (SELECT auth.uid()))))));
...
CREATE POLICY card_insert_vivacity_staff ON public.client_audit_response_documents AS PERMISSIVE FOR INSERT TO public WITH CHECK ((is_vivacity_team_safe((SELECT auth.uid())) AND (EXISTS ( SELECT 1 FROM ((client_audit_responses r JOIN client_audits a ON ((a.id = r.audit_id))) JOIN documents d ON ((d.id = client_audit_response_documents.document_id))) WHERE ((r.id = client_audit_response_documents.response_id) AND (d.tenant_id = a.subject_tenant_id))))));
...
CREATE POLICY card_update_vivacity_staff ON public.client_audit_response_documents AS PERMISSIVE FOR UPDATE TO public USING (is_vivacity_team_safe((SELECT auth.uid()))) WITH CHECK ((is_vivacity_team_safe((SELECT auth.uid())) AND (EXISTS ( SELECT 1 FROM ((client_audit_responses r JOIN client_audits a ON ((a.id = r.audit_id))) JOIN documents d ON ((d.id = client_audit_response_documents.document_id))) WHERE ((r.id = client_audit_response_documents.response_id) AND (d.tenant_id = a.subject_tenant_id))))));
...
CREATE POLICY client_email_queue_superadmin_all ON public.client_email_queue AS PERMISSIVE FOR ALL TO public USING ((EXISTS ( SELECT 1 FROM users WHERE ((users.user_uuid = (SELECT auth.uid())) AND (users.unicorn_role = 'Super Admin'::unicorn_role)))));

-- client_impact_reports
DROP POLICY IF EXISTS client_impact_reports_vivacity_update ON public.client_impact_reports;
CREATE POLICY client_impact_reports_vivacity_update ON public.client_impact_reports AS PERMISSIVE FOR UPDATE TO public USING ((user_has_tenant_access(tenant_id) AND (EXISTS ( SELECT 1 FROM users u WHERE ((u.user_uuid = (SELECT auth.uid())) AND (u.unicorn_role = ANY (ARRAY['Super Admin'::unicorn_role, 'Admin'::unicorn_role])))))));
...
CREATE POLICY client_package_stage_state_tenant_select ON public.client_package_stage_state AS PERMISSIVE FOR SELECT TO authenticated USING ((tenant_id IN ( SELECT users.tenant_id FROM users WHERE (users.user_uuid = (SELECT auth.uid())))));

DROP POLICY IF EXISTS client_package_stage_state_tenant_update ON public.client_package_stage_state;
CREATE POLICY client_package_stage_state_tenant_update ON public.client_package_stage_state AS PERMISSIVE FOR UPDATE TO authenticated USING ((tenant_id IN ( SELECT users.tenant_id FROM users WHERE (users.user_uuid = (SELECT auth.uid())))));

-- client_package_stages
DROP POLICY IF EXISTS client_package_stages_superadmin_all ON public.client_package_stages;
CREATE POLICY client_package_stages_superadmin_all ON public.client_package_stages AS PERMISSIVE FOR ALL TO public USING ((EXISTS ( SELECT 1 FROM users WHERE ((users.user_uuid = (SELECT auth.uid())) AND (users.unicorn_role = 'Super Admin'::unicorn_role)))));

DROP POLICY IF EXISTS client_package_stages_tenant_select_own ON public.client_package_stages;
CREATE POLICY client_package_stages_tenant_select_own ON public.client_package_stages AS PERMISSIVE FOR SELECT TO public USING ((EXISTS ( SELECT 1 FROM (client_packages cp JOIN tenant_members tm ON ((tm.tenant_id = cp.tenant_id))) WHERE ((cp.id = client_package_stages.client_package_id) AND (tm.user_id = (SELECT auth.uid()))))));

-- client_packages
DROP POLICY IF EXISTS client_packages_superadmin_all ON public.client_packages;
CREATE POLICY client_packages_superadmin_all ON public.client_packages AS PERMISSIVE FOR ALL TO public USING ((EXISTS ( SELECT 1 FROM users WHERE ((users.user_uuid = (SELECT auth.uid())) AND (users.unicorn_role = 'Super Admin'::unicorn_role)))));

DROP POLICY IF EXISTS client_packages_tenant_select_own ON public.client_packages;
CREATE POLICY client_packages_tenant_select_own ON public.client_packages AS PERMISSIVE FOR SELECT TO public USING ((EXISTS ( SELECT 1 FROM tenant_members tm WHERE ((tm.user_id = (SELECT auth.uid())) AND (tm.tenant_id = client_packages.tenant_id)))));
...
CREATE POLICY client_stage_documents_superadmin_all ON public.client_stage_documents AS PERMISSIVE FOR ALL TO public USING ((EXISTS ( SELECT 1 FROM users WHERE ((users.user_uuid = (SELECT auth.uid())) AND (users.unicorn_role = 'Super Admin'::unicorn_role)))));

DROP POLICY IF EXISTS client_stage_documents_tenant_select_own ON public.client_stage_documents;
CREATE POLICY client_stage_documents_tenant_select_own ON public.client_stage_documents AS PERMISSIVE FOR SELECT TO public USING ((EXISTS ( SELECT 1 FROM ((client_package_stages cps JOIN client_packages cp ON ((cp.id = cps.client_package_id))) JOIN tenant_members tm ON ((tm.tenant_id = cp.tenant_id))) WHERE ((cps.id = client_stage_documents.client_package_stage_id) AND (tm.user_id = (SELECT auth.uid()))))));
...
CREATE POLICY cti_update_via_canonical_helper ON public.client_task_instances AS PERMISSIVE FOR UPDATE TO authenticated USING ((stageinstance_id IN ( SELECT si.id FROM (stage_instances si JOIN package_instances pi ON ((pi.id = si.packageinstance_id))) WHERE app.user_can_access_tenant(pi.tenant_id)))) WITH CHECK ((tenant_is_writeable(stage_instance_tenant_id(stageinstance_id)) OR is_super_admin_safe((SELECT auth.uid()))));

-- client_team_tasks
DROP POLICY IF EXISTS client_team_tasks_superadmin_all ON public.client_team_tasks;
CREATE POLICY client_team_tasks_superadmin_all ON public.client_team_tasks AS PERMISSIVE FOR ALL TO public USING ((EXISTS ( SELECT 1 FROM users WHERE ((users.user_uuid = (SELECT auth.uid())) AND (users.unicorn_role = 'Super Admin'::unicorn_role)))));

DROP POLICY IF EXISTS client_team_tasks_tenant_select_own ON public.client_team_tasks;
CREATE POLICY client_team_tasks_tenant_select_own ON public.client_team_tasks AS PERMISSIVE FOR SELECT TO public USING ((EXISTS ( SELECT 1 FROM ((client_package_stages cps JOIN client_packages cp ON ((cp.id = cps.client_package_id))) JOIN tenant_members tm ON ((tm.tenant_id = cp.tenant_id))) WHERE ((cps.id = client_team_tasks.client_package_stage_id) AND (tm.user_id = (SELECT auth.uid()))))));
...
CREATE POLICY client_tga_snapshot_select_scoped ON public.client_tga_snapshot AS PERMISSIVE FOR SELECT TO authenticated USING ((is_vivacity_team_safe((SELECT auth.uid())) OR (EXISTS ( SELECT 1 FROM tga_links tl WHERE ((tl.client_id = client_tga_snapshot.client_id) AND has_tenant_access_safe(tl.tenant_id, (SELECT auth.uid())))))));

-- client_timeline_events
DROP POLICY IF EXISTS client_timeline_events_client_insert_visible ON public.client_timeline_events;
CREATE POLICY client_timeline_events_client_insert_visible ON public.client_timeline_events AS PERMISSIVE FOR INSERT TO public WITH CHECK (((visibility = 'client'::text) AND (EXISTS ( SELECT 1 FROM users u WHERE ((u.user_uuid = (SELECT auth.uid())) AND (u.tenant_id = client_timeline_events.tenant_id))))));

DROP POLICY IF EXISTS client_timeline_events_client_select_visible ON public.client_timeline_events;
CREATE POLICY client_timeline_events_client_select_visible ON public.client_timeline_events AS PERMISSIVE FOR SELECT TO public USING (((visibility = 'client'::text) AND (EXISTS ( SELECT 1 FROM users u WHERE ((u.user_uuid = (SELECT auth.uid())) AND (u.tenant_id = client_timeline_events.tenant_id))))));

DROP POLICY IF EXISTS client_timeline_events_vivacity_insert ON public.client_timeline_events;
CREATE POLICY client_timeline_events_vivacity_insert ON public.client_timeline_events AS PERMISSIVE FOR INSERT TO public WITH CHECK ((EXISTS ( SELECT 1 FROM users u WHERE ((u.user_uuid = (SELECT auth.uid())) AND (u.unicorn_role = ANY (ARRAY['Super Admin'::unicorn_role, 'Team Leader'::unicorn_role, 'Team Member'::unicorn_role]))))));

DROP POLICY IF EXISTS client_timeline_events_vivacity_select ON public.client_timeline_events;
CREATE POLICY client_timeline_events_vivacity_select ON public.client_timeline_events AS PERMISSIVE FOR SELECT TO public USING (((EXISTS ( SELECT 1 FROM users u WHERE ((u.user_uuid = (SELECT auth.uid())) AND (u.unicorn_role = ANY (ARRAY['Super Admin'::unicorn_role, 'Team Leader'::unicorn_role, 'Team Member'::unicorn_role]))))) AND (EXISTS ( SELECT 1 FROM users u WHERE ((u.user_uuid = (SELECT auth.uid())) AND ((u.tenant_id = client_timeline_events.tenant_id) OR (u.unicorn_role = ANY (ARRAY['Super Admin'::unicorn_role, 'Team Leader'::unicorn_role]))))))));
