-- Update accept_invitation_v2 to also sync unicorn_role to users table
-- This ensures the invited role is preserved through signup
...
-- Update comment
COMMENT ON FUNCTION public.accept_invitation_v2 IS 'Atomically accepts an invitation: validates token, creates tenant membership, syncs unicorn_role to users table, and marks invitation as accepted. Returns JSON with ok, code, and details.';