-- Update RPC function to work with tenant_id instead of client_id
-- This aligns with the current UI which uses tenant_id

-- Function to sync a tenant's TGA data from local dataset
CREATE OR REPLACE FUNCTION public.tga_sync_tenant(
  p_tenant_id bigint,
...
BEGIN
  -- Get client_id from clients_legacy for this tenant
  SELECT id, rtoid INTO v_client_id, v_rto
  FROM public.clients_legacy
  WHERE tenant_id = p_tenant_id
  LIMIT 1;

  -- Use provided rto_number if given
  IF p_rto_number IS NOT NULL AND p_rto_number != '' THEN
    v_rto := p_rto_number;
  END IF;
  ...
  IF v_rto IS NULL OR v_rto = '' THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'No RTO number found for tenant'
    );
  END IF;

  -- If no client record exists, we can't link
  IF v_client_id IS NULL THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'No client record found for tenant'
    );
  END IF;
  ...
  IF v_rto_data IS NULL THEN
    ...
    VALUES (v_client_id, v_rto, false, 'not_found', 'failed', 'RTO not found in dataset. Import dataset first.')
    ON CONFLICT (client_id) DO UPDATE SET
      rto_number = EXCLUDED.rto_number,
      is_linked = false,
      link_status = 'not_found',
      last_sync_status = 'failed',
      last_sync_error = 'RTO not found in dataset. Import dataset first.',
      updated_at = now()
  ...
  VALUES (
    v_client_id, v_rto, v_rto_data.status, v_rto_data.registration_end,
  ...
  INSERT INTO public.client_audit_log (
    ...
    p_tenant_id, 'tga_link', v_link_id::text, 'tga.sync.completed', auth.uid(),
    jsonb_build_object(
      'rto_number', v_rto,
...
-- Function to get tenant TGA data
CREATE OR REPLACE FUNCTION public.tga_get_tenant_data(p_tenant_id bigint)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  -- Get scope items (limit to prevent huge payloads)
  SELECT jsonb_agg(item) INTO v_scope_items
  FROM (
...
GRANT EXECUTE ON FUNCTION public.tga_sync_tenant(bigint, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.tga_get_tenant_data(bigint) TO authenticated;