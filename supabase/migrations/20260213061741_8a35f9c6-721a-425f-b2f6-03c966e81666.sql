...
-- ============================================================
-- 1. Add rock_type column
-- ============================================================
ALTER TABLE public.eos_rocks
ADD COLUMN IF NOT EXISTS rock_type text NOT NULL DEFAULT 'general';

ALTER TABLE public.eos_rocks
ADD CONSTRAINT eos_rocks_rock_type_check
CHECK (rock_type IN ('general', 'client'));

-- ============================================================
-- 2. Unique index: one client team rock per team+quarter+client
-- ============================================================
CREATE UNIQUE INDEX IF NOT EXISTS ux_team_client_rock
ON public.eos_rocks (function_id, quarter_year, quarter_number, client_id, rock_type)
WHERE rock_level = 'team' AND rock_type = 'client' AND archived_at IS NULL;

-- ============================================================
-- 3. RPC: upsert_rock_with_parenting
-- Handles auto-create of client team rocks and auto-suggest parent
-- ============================================================
...
BEGIN
  ...
  IF v_scope = 'individual' AND v_function_id IS NOT NULL 
    ...
    IF v_client_id IS NOT NULL THEN
      -- Look for existing client team rock
      ...
      IF v_team_rock_id IS NULL THEN
        -- Determine team lead owner
        -- 1. Try seat assignment with Primary type for this function
        ...
        -- Auto-create the client team rock
      ...
      -- Set parent if user didn't explicitly pick one
      IF v_parent_id IS NULL THEN
        v_parent_id := v_team_rock_id;
      END IF;

    ELSE
      -- Non-client individual rock: suggest general team rock as parent
      IF v_parent_id IS NULL THEN
  ...
  -- Audit log
  -- ============================================================
  INSERT INTO audit_eos_events (tenant_id, user_id, entity, entity_id, action, details)
  VALUES (
    v_tenant_id,
    auth.uid(),
    'rock',
    v_rock_id::text,
    CASE WHEN v_result->>'action' = 'created' THEN 'created' ELSE 'updated' END,
    jsonb_build_object(
      'rock_level', v_scope,
      'rock_type', v_rock_type,
      'function_id', v_function_id,
