-- Dry-run / apply consolidation of The Compliance Lab's 40 AI-generated
-- segment-courses into 6 parent courses (one per source recording).
--
-- Default is dry-run. Do NOT call with p_dry_run = false until Angela
-- confirms parent titles and the Outcome Standards (efe1f1a2) keep-list.
-- That group is reported only and never written by this function.
--
-- Apply sequence per group (when p_dry_run = false):
--   1. Insert parent course as draft (avoids autoenrol-on-publish)
--   2. One module + one lesson per segment (bounds on academy_lessons)
--   3. Unpublished combined assessment (questions preserved, not sprung live)
--   4. Dedupe enrollments onto the parent (earliest enrolled_at; status stays active)
--   5. Copy lesson progress onto the new lesson / parent enrollment
--   6. Repoint pdp_evidence_items.source_enrollment_id
--   7. Delete source enrollments; archive source courses; unpublish children
--   8. Publish the parent if any source course was published (autoenrol backfills)
...
begin
  ...
  loop
    ...
    if not p_dry_run then
      ...
      select distinct r.package_id, v_parent_id, true, r.created_by
      from public.academy_package_course_rules r
      where r.course_id = any(v_old_ids)
...
comment on function public.fn_consolidate_compliance_lab_segments(boolean) is
  'Consolidate The Compliance Lab AI segment-courses into one parent course per recording. Default dry-run. Do not call with false until Angela confirms titles and the Outcome Standards keep-list.';
