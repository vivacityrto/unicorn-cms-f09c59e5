...
BEGIN
  ...
  JOIN public.stages s             ON s.id::bigint = si.stage_id::bigint
...
BEGIN
  ...
  sp AS (
    SELECT et.tenant_id,
           COALESCE(s.provisioning_status = 'success' OR s.validation_status = 'valid', false) AS has_shared,
           (s.governance_folder_item_id IS NOT NULL) AS has_governance
    FROM etenants et
    LEFT JOIN public.tenant_sharepoint_settings s ON s.tenant_id = et.tenant_id
  )
  SELECT
