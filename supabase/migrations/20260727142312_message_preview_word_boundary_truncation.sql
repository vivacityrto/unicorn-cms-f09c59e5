-- fn_tm_on_message_insert previously truncated message previews with a hard
-- left(body, 200) cut, which can split a word mid-character (e.g. "bounced"
-- -> "bo"/"unced") with no ellipsis to signal truncation. This adds a
-- word-boundary-aware truncate helper and switches both preview writes
-- (tenant_conversations.last_message_preview and the notification message)
-- to use it. Forward-looking only — existing already-truncated preview rows
-- are not backfilled by this migration.

CREATE OR REPLACE FUNCTION public.fn_truncate_preview(input text, max_len integer DEFAULT 200)
RETURNS text
LANGUAGE sql
IMMUTABLE
SET search_path = ''
AS $function$
  SELECT CASE
    ...
    ELSE coalesce(
      nullif(rtrim(regexp_replace(left(input, max_len), '\S*$', '')), ''),
      left(input, max_len)
    ) || '…'
  END;
$function$;

REVOKE ALL ON FUNCTION public.fn_truncate_preview(text, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_truncate_preview(text, integer) TO authenticated, service_role;
...
BEGIN
  UPDATE public.tenant_conversations
     SET last_message_at      = NEW.created_at,
         last_message_preview = public.fn_truncate_preview(NEW.body, 200),
         updated_at           = now()
  ...
  BEGIN
    ...
    LOOP
      ...
      VALUES (
        ...
        'message',
        _participant.title,
        public.fn_truncate_preview(NEW.body, 200),
