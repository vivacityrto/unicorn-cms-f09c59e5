-- ============================================================
-- EOS Meeting Overhaul — Migration 16 (required_seats zero-resolved fallback)
...
-- project convention.
--
-- Gap found by an Opus plan-conformance review of the M10-M15 bugbot
-- batch, requested by Carl specifically to check those fixes against
-- unicorn-kb/handoffs/eos-meeting-overhaul-plan.md. M15's whole_roster
-- fallback (for a required_seats Configuration with no seats assigned)
-- keyed off array emptiness only: COALESCE(array_length(required_seat_ids,
-- 1), 0) > 0. That's true whenever the array is non-empty, regardless of
-- whether any of those seat IDs actually resolve to a live Primary
-- assignment. If required_seat_ids is populated (e.g. Same_Page's
-- Visionary + Integrator seats) but both happen to be simultaneously
-- vacant/expired, the required_seats branch runs, the JOIN against
-- accountability_seat_assignments produces zero rows, and no fallback
-- fires - the exact "meeting can never be started" failure mode M15 was
-- meant to close, just reached via a different door. Low probability
-- (needs a genuine double-vacancy), but defeats the self-healing intent
-- for the case the fallback exists to cover.
--
-- Fix: drive the fallback off "the required_seats derivation actually
-- produced zero attendee rows" (checked via GET DIAGNOSTICS / row count),
-- not off array length alone, in both seed_meeting_attendees() and
...
DECLARE
  v_config RECORD;
  v_required_seats_inserted int := 0;
BEGIN
  ...
  IF v_config.id IS NOT NULL THEN
    IF v_config.participant_model = 'required_seats' AND COALESCE(array_length(v_config.required_seat_ids, 1), 0) > 0 THEN
      ...
      GET DIAGNOSTICS v_required_seats_inserted = ROW_COUNT;
    END IF;

    IF v_config.participant_model <> 'required_seats'
       OR COALESCE(array_length(v_config.required_seat_ids, 1), 0) = 0
       OR v_required_seats_inserted = 0
    THEN
      -- whole_roster, a required_seats Configuration whose seats haven't
      -- been assigned yet, or a required_seats Configuration whose seats
      -- are assigned but none currently resolve to a live holder (e.g.
      -- both simultaneously vacant/expired) - fall back to the full
...
BEGIN
  ...
  THEN
    -- whole_roster, a required_seats Configuration whose seats haven't
    -- been assigned yet, or one whose seats are assigned but none
    -- currently resolve to a live holder - fall back to the full roster
    -- so Sync can never leave a meeting with zero attendees.
    INSERT INTO public.eos_meeting_attendees (meeting_id, user_id, role_in_meeting, attendance_status)
