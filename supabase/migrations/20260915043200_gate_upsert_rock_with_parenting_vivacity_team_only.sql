-- Security fix (Carl, 2026-09-15, found during RBAC v6 eos.rocks.own.manage
-- investigation): upsert_rock_with_parenting is SECURITY DEFINER, owned by
-- postgres (the table owner -- eos_rocks has relforcerowsecurity=false, so
-- RLS does not apply to writes performed by this function), granted EXECUTE
-- to `authenticated`, and had ZERO internal authorization check. Any
-- authenticated user -- not just Vivacity internal staff -- could call this
-- RPC directly to create or edit any EOS rock in Vivacity's own tenant
-- (6372), completely bypassing both the UI's disabled-button gate and the
-- eos_rocks RLS policies (which only apply to direct table writes, not to
-- this function's internal writes as the table owner).
--
-- Fix: add the same is_vivacity_team_safe(auth.uid()) gate already used
-- elsewhere in this codebase (e.g. seed_meeting_attendees_from_roles,
-- sync_l10_meeting_participants) for equivalent EOS staff-only RPCs. This
-- does not change behavior for any legitimate caller -- every real UI
-- consumer of this RPC is already gated behind the EOS route guard
-- (canAccessEOS(), Vivacity internal staff only).
...
BEGIN
  ...
  v_client_id := (p_payload->>'client_tenant_id')::integer;
  ...
  IF v_scope = 'individual' AND v_function_id IS NOT NULL
    ...
    IF v_client_id IS NOT NULL THEN
      ...
      WHERE rock_level = 'team'
        ...
        AND client_tenant_id = v_client_id
      ...
      IF v_team_rock_id IS NULL THEN
        ...
        INSERT INTO eos_rocks (
          ...
          function_id, owner_id, client_tenant_id,
