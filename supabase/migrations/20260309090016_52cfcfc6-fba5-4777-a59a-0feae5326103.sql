...
BEGIN
  ...
  -- Calculate total used minutes scoped to renewal year, matching on package_instance_id
  SELECT 
  ...
  WHERE te.tenant_id = v_tenant_id
    AND te.package_id = p_client_package_id
    AND te.created_at >= v_renewal_start
    AND te.created_at < v_renewal_end;

  -- Calculate trailing 30-day usage (also scoped to instance)
  SELECT 
    COALESCE(SUM(te.duration_minutes), 0),
    COALESCE(SUM(CASE WHEN te.source = 'manual' THEN te.duration_minutes ELSE 0 END), 0),
    COALESCE(SUM(CASE WHEN te.source = 'timer' THEN te.duration_minutes ELSE 0 END), 0),
