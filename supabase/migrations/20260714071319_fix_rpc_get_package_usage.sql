...
BEGIN
  ...
  -- FIXED: was joining on te.package_id (no FK, legacy - not the same value space as
  -- package_instances.id). Now uses te.package_instance_id, excludes carry_over adjustments
  -- from "used", and includes child instances via parent_instance_id (matches the
  -- fn_package_used_minutes rollup convention used elsewhere).
  SELECT
