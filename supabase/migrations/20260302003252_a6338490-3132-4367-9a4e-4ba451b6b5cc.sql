...
-- Only staff_task_instances lacks an identity/sequence on id
CREATE SEQUENCE IF NOT EXISTS staff_task_instances_id_seq;
SELECT setval('staff_task_instances_id_seq', GREATEST(205017, COALESCE((SELECT MAX(id) FROM staff_task_instances), 0) + 1));
ALTER TABLE staff_task_instances ALTER COLUMN id SET DEFAULT nextval('staff_task_instances_id_seq');

-- Drop and recreate the start_client_package RPC
DROP FUNCTION IF EXISTS start_client_package(bigint, bigint, uuid);

CREATE OR REPLACE FUNCTION start_client_package(
  p_tenant_id bigint,
  p_package_id bigint,
...
BEGIN
  ...
  LOOP
    ...
    -- 2c. Emails (scoped by stage_id AND package_id)
    ...
    -- 2d. Documents (scoped by stage_id only)
    INSERT INTO document_instances (document_id, stageinstance_id, tenant_id, status, isgenerated)
    SELECT sd.document_id, v_stage_instance_id, p_tenant_id, 'pending', false
      FROM stage_documents sd
     WHERE sd.stage_id = v_stage.stage_id;
