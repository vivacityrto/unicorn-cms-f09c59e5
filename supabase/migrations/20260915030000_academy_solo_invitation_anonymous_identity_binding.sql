...
BEGIN
  ...
  -- New Supabase signups may have authData.user without a session while
  -- email confirmation is pending. In that anonymous case, bind the caller
  -- to the auth identity belonging to the invited email before any writes.
  IF (SELECT auth.uid()) IS NULL THEN
    SELECT id INTO v_invited_auth_uuid
    FROM auth.users
    WHERE lower(email) = lower(v_invitation.email)
    LIMIT 1;

    IF v_invited_auth_uuid IS NULL OR v_invited_auth_uuid <> p_user_id THEN
      RETURN jsonb_build_object('ok', false, 'code', 'IDENTITY_MISMATCH',
        'message', 'Invitation can only be accepted by the invited user');
    END IF;
  END IF;
...
-- The fresh-signup path can be temporarily anonymous until email confirmation;
-- the function body above binds p_user_id to the invited email's auth identity.
REVOKE ALL ON FUNCTION public.accept_invitation_v2(text, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.accept_invitation_v2(text, uuid) TO anon, authenticated, service_role;
