-- ============================================================
-- EOS Meeting Overhaul — Migration 17 (validate_meeting_close ratings formula)
...
-- project convention.
--
-- Gap found by Cursor Bugbot review on PR #39: M12 changed
-- close_meeting_with_validation's required-ratings formula from
-- GREATEST(1, FLOOR(present * 0.5)) to CEIL(present * 0.5), but missed
-- a second, separate function - validate_meeting_close(uuid) - that the
-- close dialog's live preview/checklist calls (MeetingCloseValidationDialog
-- -> useMeetingOutcomes' validateClose -> supabase.rpc('validate_meeting_close')).
-- Its own header comment explicitly says "Ratings check (aligned with
-- close_meeting_with_validation)" - it was designed to stay in lockstep
-- with the actual close-gate formula, and M12 broke that lockstep by
-- only updating one of the two. Consequence: the dialog could show
-- "ready to close" (old, more lenient formula) and then the real close
-- attempt fails the newer, stricter CEIL formula - a confusing
-- ready-then-rejected UX exactly like the close-dialog inconsistency
-- M6/M12 were meant to eliminate.
--
-- Fix: same CEIL(present * 0.5) change, applied to
-- validate_meeting_close, nothing else touched.
-- ============================================================
...
BEGIN
  ...
  -- Ratings check (aligned with close_meeting_with_validation, M12/M17)
