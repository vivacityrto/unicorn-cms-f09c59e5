...
Deno.serve(async (req) => {
  ...
  if (!draftLogId || !auditId) {
    return json({ error: 'draft_log_id and audit_id are required' }, 400);
  }
  ...
  for (const key of ['executive_summary', 'overall_finding', 'risk_rationale'] as const) {
    ...
    if (typeof parsed === 'string') {
      return json({ error: `${key}: ${parsed}` }, 400);
    }
    fields[key] = parsed;
  }
  if (Object.values(fields).every((v) => v === null)) {
    return json({ error: 'At least one field decision is required' }, 400);
  }
  ...
  if (d.action !== 'ai.executive_summary_drafted') {
    return json({ error: 'Referenced log row is not an AI executive summary draft' }, 400);
  }
  if (d.actor_user_id !== callerUserId) {
    return json({ error: 'Draft log belongs to a different user' }, 403);
  }
  if (d.entity_id !== auditId) {
    return json({ error: 'Draft log does not match this audit' }, 400);
  }
