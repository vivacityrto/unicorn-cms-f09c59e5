...
serve(async (req) => {
  ...
  // Nightly cron-only job (see cron.job "run-tenant-risk-forecast-nightly"),
  // triggers a full-tenant recompute -- had no auth check at all until now,
  // despite the cron job already sending x-cron-invoke-secret.
