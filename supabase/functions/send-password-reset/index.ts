...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { data: authUserData, error: authUserError } = await supabaseAdmin.auth.admin.getUserById(user_uuid);

    if (authUserError || !authUserData?.user) {
      console.error(`User ${targetUser.email} not found in auth.users`);
      return new Response(
        JSON.stringify({
          ok: false,
...
<body>
  <div class="container">
    ...
    <div class="footer">
      Vivacity • <a href="${APP_BASE_URL}">${APP_BASE_URL}</a>
    </div>
  </div>
</body>
</html>`;
