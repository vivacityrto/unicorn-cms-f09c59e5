...
Deno.serve(async (req) => {
  ...
  // Its own header comment says "can be triggered by DB webhooks or called
  // on schedule", but no pg_cron job, DB webhook trigger, or edge-function
  // caller was found anywhere for this function (2026-08-18 audit) -- same
  // "no caller ever found" situation as schedule-task-reminders. Gated on
  // the standing cron-invoke pattern rather than inventing a caller
  // identity; had no auth check at all until now.
